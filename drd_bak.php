							...
							<?php
								if (isset($_POST["proses"])&& $_POST["proses"]=="Proses"){
									...
									if(isset($_POST["rjrep"])&& $_POST["rjrep"]=="rjdetail"){
										...
										$sql="
										SELECT DISTINCT r.nomor, m.nolama, SUBSTR(lcase(m.nama),1,20), m.dkd, r.standlalu, r.standkini, r.pakai, format(r.uangair,0), format(r.adm,0), format(r.meter,0), format(r.meterai,0), format((r.uangair + r.adm + r.meter + r.meterai + r.denda),0),m.kondisi_meter,IF(r.status_bayar=1,'LNS','TGK')
										FROM rekening1 AS r
										...
										$sql_jml="
										SELECT format(SUM(r.pakai),0), format(SUM(r.uangair),0), format(SUM(r.adm),0), format(SUM(r.meter),0), format(SUM(r.meterai),0), format(SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)),0) FROM rekening1 AS r
										LEFT OUTER JOIN master AS m ON m.nomor=r.nomor
										WHERE  $slipit (r.periode BETWEEN $periode AND $periode2) AND m.putus=0 ORDER BY m.nolama;
										";
									...
									//=====================================BOF=REKAP==============================================
										...
										FROM rekening1 as r
										INNER JOIN master AS m ON m.nomor=r.nomor
										WHERE $slipit (r.periode BETWEEN $periode AND $periode2) AND m.putus=0 GROUP BY m.gol ORDER BY m.gol
										;";
										...
										FROM rekening1 as r
										INNER JOIN master AS m ON m.nomor=r.nomor
										WHERE $slipit (r.periode BETWEEN $periode AND $periode2) AND m.putus=0
										;";
										$qry0=mysql_query($sql_jml)or die("err: ".mysql_error());
										if($row0=mysql_fetch_row($qry0)){
											$jrek=$row0[0];
