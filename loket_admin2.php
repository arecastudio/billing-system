		...
		<table width="1045px" border="0" cellpadding="1" cellspacing="1" align="center" class="wrapper"  style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:11px;box-shadow: 0 0 15px #999;background-color:#f0f0f0;" background="img/grids.gif">
          <form id="fr1" name="fr1" method="post" action="" onSubmit="">
            <tr style="font:calibri;font-size:18px;font-weight:bold;color:#fff;background:linear-gradient(#666,#004);">
            	<td colspan="4" align="center"><marquee behavior="alternate" scrollamount="2" hspace="300">.::Pelunasan Transaksi Admin::.</marquee></td>
            </tr>
            <tr>
              <td width="130px">Nomor/NoLama</td>
              <td width="550px">
				<input type="text" size="13" name="txnomor" id="txnomor" maxlength="15"  onkeypress="return isNumb(event);" /><input type="button" name="cari" value="Cari!" class="kelas_tombol" onclick="window.open('cari.php','popUpWindow','height=400,width=800,left=10,top=10,scrollbars=yes,menubar=no'); return false;" />
				&nbsp;
				&nbsp;
				&nbsp;
				Loket&nbsp;
				<select name="optjlok" id="optjlok">
					...
					<?php
						$sql=mysql_query("select kode_loket,ket_loket from m_loket where kode_loket in('ABE','WNA','JPR','UJU','STN','BPD','POS','BRV','FKT','BNI') order by kode_loket");
				...
				&nbsp;
				Opr&nbsp;
				<input type="text" size="7" id="txopr" name="txopr" required />
			  </td>
			...
			<tr height="35px">
                ...
                <td style="font-size:18px;font-family:calibri;font-weight:bold;"><?php //echo date('Y-M-d');?>
					<input type="text" size="8px" name="txtgl" id="txtgl" readonly class="datepicker" value="<?php echo date('Y-m-d');?>" style="font-size:18px;font-family:calibri;font-weight:bold;" />
				</td>
			</tr>
          <!--/form-->
		  </table>
                        ...
                        <table width="100%" border="0" cellpadding="1" cellspacing="1" align="center" bgcolor="white" style="border-collapse:collapse;background:#ffd;font-family:calibri;font-size:12px;" >
							...
							<?php
								if (isset($_POST["proses"])&& $_POST["proses"]=="Tampilkan!" && $dapat==1){
									...
									//$nomor=$_POST['txnomor'];
									$nomor=trim($_POST['txnomor']);
									$opr=trim($_POST['txopr']);
									$tgl=trim($_POST['txtgl']);
									$loket=trim($_POST['optjlok']);
									$cek=$_POST["rcek"];
									//$periode=getPeriodeLalu();
									echo"<script type=\"text/javascript\">document.getElementById(\"txnomor\").value=\"$nomor\";</script>";
									echo"<script type=\"text/javascript\">document.getElementById(\"txopr\").value=\"$opr\";</script>";
									echo"<script type=\"text/javascript\">document.getElementById(\"txtgl\").value=\"$tgl\";</script>";
									echo"<script type=\"text/javascript\">document.getElementById(\"optjlok\").value=\"$loket\";</script>";
									if($cek!=""){
									...
									$total=0;
									$sql="
									SELECT r.periode,r.standlalu,r.standkini,r.pakai,r.uangair,r.adm,r.meter,r.denda,r.meterai,(r.uangair + r.adm + r.meter + r.meterai + r.denda)
									FROM rekening1 AS r								
									...
									while($rows=mysql_fetch_row($qry)){
										...
										<tr bgcolor=\"$wrn\">
											...
											<td align=\"center\" style=\"color:#f0f;font-weight:bold;font-size:16px;\">".$rows[0]."</td>
											...
											<td align=\"right\" style=\"color:#f0f;font-weight:bold;font-size:16px;\">".formatMoney($rows[9])."</td>
										</tr>
										";
										$total+=$rows[9];
									}																	
...
if (isset($_POST["lunaskan"]) && trim($_POST['txnomor'])!=""){	
	...
	while($row=mysql_fetch_row($query)){
		...
		$nmr1=trim($row[0]);		
		if(isset($_POST[$prd])){
			mysql_query("UPDATE rekening1 SET status_bayar=1,tbayar='$tgl',loket='$loket' WHERE nomor='$nomor' AND periode=$prd;")or die(mysql_error());
			mysql_query("insert IGNORE INTO transaksi(nomor,periode,total,tbayar,jbayar,loket,operator,batal)VALUES('$nomor',$prd,$total,'".date('Y-m-d)."','".date('H:i:s')."','".$loket."','"$_SESSION['nama_user']."',0);")or die(mysql_error());
$arr_prd[]=$prd;
$i++;
}
}
...
//			VALUES('$nomor',$prd,$total,'$tgl','".date('H:i:s')."','$loket','$opr',0)
//			ON DUPLICATE KEY UPDATE transaksi.tbayar='$tgl',transaksi.jbayar='".date('H:i:s')."',transaksi.loket='$loket',transaksi.operator='$opr',transaksi.batal=0
//			;";			
