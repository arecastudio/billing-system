...
?>
	...
	<tr>
		<td>
		<!--############################################################################################################-->
        <div align="center"><font face="Arial,sans-serif" size="4px"><b style="color:#eee;">Rekap Daftar Sisa Rekening [DSR] per Cabang [ UPP ]</b></font></div><hr width="1024px"/>		
					...
					<div style="height:350px;overflow:auto">
						<table width="100%" border="1" cellpadding="1" cellspacing="1" align="center" bgcolor="white" style="border-collapse:collapse;background:#ffc;font-family:calibri;font-size:12px;">
                            ...
                            if(isset($_POST['proses'])&&$_POST['proses']=='Proses'){							
								...
								$judul="Rekap DSR - Per-Tanggal [$takhir]";
								...
								WHERE $selip r.periode=$bln1 AND ((r.status_bayar=1 AND t.batal=0 AND t.tbayar>='$takhir')OR(r.status_bayar=0))
								GROUP BY m.gol;";								
								...
								INNER JOIN master AS m ON m.nomor=r.nomor
								LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode
								WHERE $selip (r.periode<$bln1 AND r.periode>$prd_batas) AND ((r.status_bayar=1 AND t.batal=0 AND t.tbayar>='$takhir')OR(r.status_bayar=0))
								...
								INNER JOIN master AS m ON m.nomor=r.nomor
								LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode
								WHERE $selip (r.periode<=$prd_batas) AND ((r.status_bayar=1 AND t.batal=0 AND t.tbayar>='$takhir')OR(r.status_bayar=0))
								GROUP BY m.gol,r.periode
								ORDER BY r.periode ASC;
								";
							...
							$sql_jml="
							SELECT DISTINCT format(count(*),0),format(SUM(r.pakai),0),format(SUM(r.uangair),0),format(SUM(r.adm),0),format(SUM(r.meter),0),format(SUM(r.meterai),0),format(SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)),0)
							FROM rekening1 AS r
							INNER JOIN master AS m ON m.nomor=r.nomor
							LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode
							WHERE $selip ((r.status_bayar=1 AND t.batal=0 AND t.tbayar>='$takhir')OR(r.status_bayar=0))
							";
