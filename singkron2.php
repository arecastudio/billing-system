...
?>
	...
	<tr>
		...
		<table width="700px" border="0" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF" align="center" class="wrapper"  style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:11px;box-shadow: 0 0 15px #999;background-color:#f0f0f0;" background="img/grids.gif">
            ...
            <tr>
              <td colspan="4" align="center" style="padding:2px 0;text-shadow:2px 2px 2px red;color:#000;font-size:18px;background: linear-gradient(#ff0, #fff);font-weight:bold;font-family:'Times New Roman', Times, serif'" ><b>Ambil DSMP dan LPP dari Cabang</b></font></td>
              </tr>
            <tr>
            	<td>Cabang</td>
                <td>
		...
		if(isset($_POST['dsmp'])){
			...
			$qry=mysql_query("SELECT DISTINCT d.nomor,d.periode,d.angka FROM dsmp0 AS d INNER JOIN master AS m on m.nomor=d.nomor WHERE d.PERIODE=$peri AND m.cabang=$cabang;")or die(mysql_error());
		...
		if(isset($_POST['master'])){
			...
			if($jml>0){
				...
				while($i<$jml){
					...
					//$qry=mysql_query("UPDATE master SET nama='$nama[$i]',alamat='".strip_tags($alamat[$i])."',kondisi_meter='$kondisi_meter[$i]',pakai_air_bln=$pakai[$i] WHERE cabang='$cabang' AND nomor='$nomor[$i]' AND nolama='$nolama[$i]';") or die(mysql_error());
					$qry=mysql_query("UPDATE master SET kondisi_meter='$kondisi_meter[$i]',pakai_air_bln=$pakai[$i] WHERE cabang='$cabang' AND nomor='$nomor[$i]' AND nolama='$nolama[$i]';") or die(mysql_error());
				}
