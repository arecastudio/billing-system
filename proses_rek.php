...
$sql2="INSERT IGNORE INTO rekening1(nomor,periode, standlalu, standkini, pakai) select a.nomor,b.periode,a.angka,b.angka,(b.angka-a.angka) FROM dsmp_new AS a, dsmp_new AS b WHERE a.nomor=b.nomor AND b.periode=$period AND a.periode=$lastperiod;";

$sql="INSERT IGNORE INTO rekening1(nomor,periode, standlalu, standkini, pakai,meter) SELECT a.nomor,b.periode,a.angka,b.angka,(b.angka-a.angka),mt.biaya FROM dsmp_new AS a, dsmp_new AS b,master AS ms,meter AS mt WHERE a.nomor=b.nomor AND b.periode=$period AND a.periode=$lastperiod AND b.nomor=ms.nomor AND ms.ukuran=mt.kode AND ms.putus=0;";
...
//perubahan pada kwm,putus,ukuran
	...
	FROM master AS m
	LEFT OUTER JOIN dsmp_new AS a ON a.nomor=m.nomor AND a.periode=$lastperiod
	LEFT OUTER JOIN dsmp_new AS b ON b.nomor=m.nomor AND b.periode=$period
	WHERE m.putus='0'
	;	
	";	
...
//baru update 12 april 2015 penambahan round untuk pembulatan uangair pada nilai ratusan
	...
	mysql_query("DELETE FROM rekening1_awal WHERE periode=(SELECT DISTINCT d1.periode FROM dsmp_new AS d1 ORDER BY d1.periode DESC LIMIT 1);")or die(mysql_error());
	...
	mysql_query("DELETE FROM lap_bulan_ti WHERE periode=(SELECT DISTINCT d1.periode FROM dsmp_new AS d1 ORDER BY d1.periode DESC LIMIT 1);")or die(mysql_error());
	...
	,(select distinct count(m2.nomor) from master as m2 inner join dsmp_new as d2 on d2.nomor=m2.nomor inner join dsmp_new as d3 on d3.nomor=m2.nomor where d3.periode=$lastperiod AND d2.periode=d1.periode and (d2.angka-d3.angka)<=10 and m2.putus='0' and m2.cabang='01')as jpr_10
	,(select distinct count(m2.nomor) from master as m2 inner join dsmp_new as d2 on d2.nomor=m2.nomor inner join dsmp_new as d3 on d3.nomor=m2.nomor where d3.periode=$lastperiod AND d2.periode=d1.periode and (d2.angka-d3.angka)>100 and m2.putus='0' and m2.cabang='01')as jpr_100
	,(select distinct count(m2.nomor) from master as m2 inner join dsmp_new as d2 on d2.nomor=m2.nomor inner join dsmp_new as d3 on d3.nomor=m2.nomor where d3.periode=$lastperiod AND d2.periode=d1.periode and (d2.angka-d3.angka)<=10 and m2.putus='0' and m2.cabang='02')as uju_10
	,(select distinct count(m2.nomor) from master as m2 inner join dsmp_new as d2 on d2.nomor=m2.nomor inner join dsmp_new as d3 on d3.nomor=m2.nomor where d3.periode=$lastperiod AND d2.periode=d1.periode and (d2.angka-d3.angka)>100 and m2.putus='0' and m2.cabang='02')as uju_100
	,(select distinct count(m2.nomor) from master as m2 inner join dsmp_new as d2 on d2.nomor=m2.nomor inner join dsmp_new as d3 on d3.nomor=m2.nomor where d3.periode=$lastperiod AND d2.periode=d1.periode and (d2.angka-d3.angka)<=10 and m2.putus='0' and m2.cabang='03')as abe_10
	,(select distinct count(m2.nomor) from master as m2 inner join dsmp_new as d2 on d2.nomor=m2.nomor inner join dsmp_new as d3 on d3.nomor=m2.nomor where d3.periode=$lastperiod AND d2.periode=d1.periode and (d2.angka-d3.angka)>100 and m2.putus='0' and m2.cabang='03')as abe_100
	,(select distinct count(m2.nomor) from master as m2 inner join dsmp_new as d2 on d2.nomor=m2.nomor inner join dsmp_new as d3 on d3.nomor=m2.nomor where d3.periode=$lastperiod AND d2.periode=d1.periode and (d2.angka-d3.angka)<=10 and m2.putus='0' and m2.cabang='04')as wna_10
	,(select distinct count(m2.nomor) from master as m2 inner join dsmp_new as d2 on d2.nomor=m2.nomor inner join dsmp_new as d3 on d3.nomor=m2.nomor where d3.periode=$lastperiod AND d2.periode=d1.periode and (d2.angka-d3.angka)>100 and m2.putus='0' and m2.cabang='04')as wna_100
	,(select distinct count(m2.nomor) from master as m2 inner join dsmp_new as d2 on d2.nomor=m2.nomor inner join dsmp_new as d3 on d3.nomor=m2.nomor where d3.periode=$lastperiod AND d2.periode=d1.periode and (d2.angka-d3.angka)<=10 and m2.putus='0' and m2.cabang='05')as stn_10
	,(select distinct count(m2.nomor) from master as m2 inner join dsmp_new as d2 on d2.nomor=m2.nomor inner join dsmp_new as d3 on d3.nomor=m2.nomor where d3.periode=$lastperiod AND d2.periode=d1.periode and (d2.angka-d3.angka)>100 and m2.putus='0' and m2.cabang='05')as stn_100	
	,(select distinct count(m2.nomor) from master as m2 inner join dsmp_new as d2 on d2.nomor=m2.nomor inner join dsmp_new as d3 on d3.nomor=m2.nomor where d3.periode=$lastperiod AND d2.periode=d1.periode and (d2.angka-d3.angka)<=10 and m2.putus='0' and m2.cabang='06')as gym_10
	,(select distinct count(m2.nomor) from master as m2 inner join dsmp_new as d2 on d2.nomor=m2.nomor inner join dsmp_new as d3 on d3.nomor=m2.nomor where d3.periode=$lastperiod AND d2.periode=d1.periode and (d2.angka-d3.angka)>100 and m2.putus='0' and m2.cabang='06')as gym_100
	...
	from dsmp_new as d1 order by d1.periode desc limit 1
	;";
...
?>
	...
	<tr>
		...
		<table align="center" border="0" width="500px" cellpadding="2" cellspacing="2" style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:11px;box-shadow: 0 0 15px #999;background-color:#f0f0f0;" background="img/grids.gif">
			...
			FROM master AS m
			INNER JOIN dsmp_new AS a ON a.nomor=m.nomor AND a.periode=$p2
			INNER JOIN dsmp_new AS b ON b.nomor=m.nomor AND b.periode=$p1
