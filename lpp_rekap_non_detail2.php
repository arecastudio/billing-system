					...
					<div style="height:350px;overflow:auto">
						<table width="100%" border="1" cellpadding="1" cellspacing="1" align="center" bgcolor="white" style="border-collapse:collapse;background:#ffc;font-family:calibri;font-size:12px;">
                            ...
                            if(isset($_POST['proses'])&&$_POST['proses']=='Proses'){							
								...
								$sql_i[0]="SELECT DISTINCT m.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,COUNT(*) as jml_rek,SUM(r.pakai) as jpakai,SUM(r.uangair) as juangair,SUM(r.adm) as jadm,SUM(r.meter) as jmeter,SUM(r.meterai) as jmeterai,SUM((r.uangair + r.adm + r.meter + r.meterai )) as jtotal,ROUND(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0) as avg_harga, ROUND(IFNULL((SUM(r.pakai)/COUNT(*)),0),0) as avg_plg,r.periode
								FROM rekening1 AS r
								INNER JOIN master AS m ON m.nomor=r.nomor
								INNER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode
								WHERE $selip r.periode=$bln0 AND (t.tbayar BETWEEN '$tawal' AND '$takhir') AND r.status_bayar=1 AND t.batal=0
								GROUP BY m.gol
								;";
								#=============================================================================================
								...
								GROUP BY m.gol
								;";
								//$sql_i[2]="SELECT DISTINCT r.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,COUNT(*) as jml_rek,SUM(r.pakai) as jpakai,SUM(r.uangair) as juangair,SUM(r.adm) as jadm,SUM(r.meter) as jmeter,SUM(r.meterai) as jmeterai,SUM((r.uangair + r.adm + r.meter + r.meterai )) as jtotal,ROUND(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0) as avg_harga, ROUND(IFNULL((SUM(r.pakai)/COUNT(*)),0),0) as avg_plg FROM rekening1 AS r LEFT OUTER JOIN master AS m ON m.nomor=r.nomor LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode WHERE (r.periode BETWEEN $bln3 AND $bln2) AND (r.tbayar BETWEEN '$tawal' AND '$takhir') AND r.status_bayar=1 GROUP BY r.gol;";
								//$sql_i[3]="SELECT DISTINCT r.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,COUNT(*) as jml_rek,SUM(r.pakai) as jpakai,SUM(r.uangair) as juangair,SUM(r.adm) as jadm,SUM(r.meter) as jmeter,SUM(r.meterai) as jmeterai,SUM((r.uangair + r.adm + r.meter + r.meterai )) as jtotal,ROUND(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0) as avg_harga, ROUND(IFNULL((SUM(r.pakai)/COUNT(*)),0),0) as avg_plg FROM rekening1 AS r LEFT OUTER JOIN master AS m ON m.nomor=r.nomor LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode WHERE (r.periode BETWEEN $bln6 AND $bln4) AND (r.tbayar BETWEEN '$tawal' AND '$takhir') AND r.status_bayar=1 GROUP BY r.gol;";
								//$sql_i[4]="SELECT DISTINCT r.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,COUNT(*) as jml_rek,SUM(r.pakai) as jpakai,SUM(r.uangair) as juangair,SUM(r.adm) as jadm,SUM(r.meter) as jmeter,SUM(r.meterai) as jmeterai,SUM((r.uangair + r.adm + r.meter + r.meterai )) as jtotal,ROUND(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0) as avg_harga, ROUND(IFNULL((SUM(r.pakai)/COUNT(*)),0),0) as avg_plg FROM rekening1 AS r LEFT OUTER JOIN master AS m ON m.nomor=r.nomor LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode WHERE (r.periode BETWEEN $bln12 AND $bln7) AND (r.tbayar BETWEEN '$tawal' AND '$takhir') AND r.status_bayar=1 GROUP BY r.gol;";
								
								//$sql_i[5]="SELECT DISTINCT r.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,COUNT(*) as jml_rek,SUM(r.pakai) as jpakai,SUM(r.uangair) as juangair,SUM(r.adm) as jadm,SUM(r.meter) as jmeter,SUM(r.meterai) as jmeterai,SUM((r.uangair + r.adm + r.meter + r.meterai )) as jtotal,ROUND(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0) as avg_harga, ROUND(IFNULL((SUM(r.pakai)/COUNT(*)),0),0) as avg_plg FROM rekening1 AS r LEFT OUTER JOIN master AS m ON m.nomor=r.nomor LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode WHERE (r.periode BETWEEN $bln24 AND $bln13) AND (r.tbayar BETWEEN '$tawal' AND '$takhir') AND r.status_bayar=1 GROUP BY r.gol;";
								//$sql_i[6]="SELECT DISTINCT r.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,COUNT(*) as jml_rek,SUM(r.pakai) as jpakai,SUM(r.uangair) as juangair,SUM(r.adm) as jadm,SUM(r.meter) as jmeter,SUM(r.meterai) as jmeterai,SUM((r.uangair + r.adm + r.meter + r.meterai )) as jtotal,ROUND(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0) as avg_harga, ROUND(IFNULL((SUM(r.pakai)/COUNT(*)),0),0) as avg_plg FROM rekening1 AS r LEFT OUTER JOIN master AS m ON m.nomor=r.nomor LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode WHERE (r.periode BETWEEN $bln36 AND $bln25) AND (r.tbayar BETWEEN '$tawal' AND '$takhir') AND r.status_bayar=1 GROUP BY r.gol;";
								//$sql_i[7]="SELECT DISTINCT r.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,COUNT(*) as jml_rek,SUM(r.pakai) as jpakai,SUM(r.uangair) as juangair,SUM(r.adm) as jadm,SUM(r.meter) as jmeter,SUM(r.meterai) as jmeterai,SUM((r.uangair + r.adm + r.meter + r.meterai )) as jtotal,ROUND(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0) as avg_harga, ROUND(IFNULL((SUM(r.pakai)/COUNT(*)),0),0) as avg_plg FROM rekening1 AS r LEFT OUTER JOIN master AS m ON m.nomor=r.nomor LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode WHERE (r.periode BETWEEN $prd_batas AND $bln37) AND (r.tbayar BETWEEN '$tawal' AND '$takhir') AND r.status_bayar=1 GROUP BY r.gol;";
								...
								//$sql_i[3]="SELECT DISTINCT m.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,COUNT(*) as jml_rek,SUM(r.pakai) as jpakai,SUM(r.uangair) as juangair,SUM(r.adm) as jadm,SUM(r.meter) as jmeter,SUM(r.meterai) as jmeterai,SUM((r.uangair + r.adm + r.meter + r.meterai )) as jtotal,ROUND(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0) as avg_harga, ROUND(IFNULL((SUM(r.pakai)/COUNT(*)),0),0) as avg_plg,r.periode FROM rekening1 AS r INNER JOIN master AS m ON m.nomor=r.nomor LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode WHERE $selip r.periode<=$prd_batas AND (r.tbayar BETWEEN '$tawal' AND '$takhir') AND r.status_bayar=1 AND t.batal=0 GROUP BY m.gol;";
								//$sql_i[7]="SELECT DISTINCT m.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,COUNT(*) as jml_rek,SUM(r.pakai) as jpakai,SUM(r.uangair) as juangair,SUM(r.adm) as jadm,SUM(r.meter) as jmeter,SUM(r.meterai) as jmeterai,SUM((r.uangair + r.adm + r.meter + r.meterai )) as jtotal,ROUND(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0) as avg_harga, ROUND(IFNULL((SUM(r.pakai)/COUNT(*)),0),0) as avg_plg FROM rekening1 r, master m WHERE r.periode<=$bln37 AND (r.tbayar LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode BETWEEN '$tawal' AND '$takhir') AND m.nomor=r.nomor and r.status_bayar=1 GROUP BY m.gol;";
								$sql_i[3]="
								SELECT DISTINCT m.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,COUNT(*) as jml_rek,SUM(r.pakai) as jpakai,SUM(r.uangair) as juangair,SUM(r.adm) as jadm,SUM(r.meter) as jmeter,SUM(r.meterai) as jmeterai,SUM((r.uangair + r.adm + r.meter + r.meterai )) as jtotal,ROUND(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0) as avg_harga, ROUND(IFNULL((SUM(r.pakai)/COUNT(*)),0),0) as avg_plg,r.periode
								FROM rekening1 AS r
								INNER JOIN master AS m ON m.nomor=r.nomor
								...
								$cnt1[]="";$cnt2[]="";$baris="";
								while($j<=3){
									if ($j==0) {$bts=$bln0;$peri="Bulan Akhir";}
