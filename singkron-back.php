...
?>
	...
	<tr>
		...
		<table width="700px" border="0" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF" align="center" class="wrapper"  style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:11px;box-shadow: 0 0 15px #999;background-color:#f0f0f0;" background="img/grids.gif">
            ...
            <tr>
                ...
                <select name="optperiode" >
                  	...
                  	<?php
						$sql=mysql_query("select distinct periode from dsmp0 order by periode desc limit 2;");
		...
		if(isset($_POST['dsmp'])){
			...
			$qry=mysql_query("SELECT DISTINCT d.nomor,d.periode,d.angka,m.kondisi_meter FROM dsmp_new AS d INNER JOIN master AS m on m.nomor=d.nomor WHERE d.PERIODE=$peri AND d.angka>=0 AND m.cabang=$cabang AND m.kondisi_meter<>'2';")or die(mysql_error());
			...
			if($jml>0){
				...
				while($i<$jml){
					...
					$qry=mysql_query("INSERT IGNORE INTO dsmp_new(nomor,periode,angka)VALUES('$nomor[$i]',$periode[$i],$angka[$i]) ON DUPLICATE KEY UPDATE angka=$angka[$i];") or die(mysql_error());
					...
					if($kwm[$i]=='3'||$kwm[$i]=='4'){
						...
						$qry=mysql_query("UPDATE dsmp_new SET angka=$angka[$i] WHERE nomor='$nomor[$i]' AND periode=$periode[$i] AND periode<>$plalu;")or die(mysql_error());
					}
					//echo"Hasil: $nomor[$i] - $periode[$i] -  $angka[$i]<br/>";
				}
