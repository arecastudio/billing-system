...
<script type="text/javascript" language="javascript">
	...
	<tr>
		<td><br>
            ...
            <h3 align="center" style="text-shadow:4px -4px 2px black;color:#fff;">Lihat Pelanggan</h3><hr width="600px" align="center" />            
            <table width="880px" border="0" cellpadding="1" cellspacing="2" align="center" class="wrapper"  style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:12px;box-shadow: 0 0 15px #999;" background="img/grids.gif">
                ...
                if(isset($_POST['cari']) && $_POST['txnomor']!=''){
					...
					WHERE (m.nomor='$nmr' OR m.nolama='$nmr') AND (m.putus='0' OR m.putus='3' OR m.putus='5');";
					...
					if($row=mysql_fetch_row($qry)){
						...
						";						
						$sql="SELECT DISTINCT r.periode, FORMAT(r.standlalu,0), FORMAT(r.standkini,0), FORMAT (r.pakai,0), FORMAT(r.uangair,0), FORMAT(r.adm,0), FORMAT(r.meter,0), FORMAT(r.denda,0), IFNULL(FORMAT(r.meterai,0),0), IFNULL(FORMAT(r.total,0),0),IF(r.status_bayar=0,'TUNGGAK','LUNAS'),IF(r.status_bayar=1,IF(t.tbayar<>'0000-00-00',t.tbayar,r.tbayar),'-'),IF(r.status_bayar=1,IF(t.loket<>'',t.loket,r.loket),'-'),IF(r.status_bayar=1,UCASE(t.operator),'-')
						FROM rekening1 AS r
