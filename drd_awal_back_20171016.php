...
switch($bayar){
	...
	default:echo"<script type=\"text/javascript\">document.getElementById(\"optbayar\").value=\"$bayar\"</script>";break;
									...
									if(isset($_POST["rjrep"])&& $_POST["rjrep"]=="rjdetail"){
										...
										LEFT OUTER JOIN master AS m ON m.nomor=r.nomor
										WHERE  $slipit (r.periode BETWEEN $periode AND $periode2) AND m.putus=0 AND ".$sql_pakai." AND ".$sql_bayar." ORDER BY m.nolama;
										";
									...
									//=====================================BOF=REKAP==============================================
										...
										$sql="
										SELECT DISTINCT r.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol)as ket,FORMAT(COUNT(*),0) as jml_rek,FORMAT(SUM(r.pakai),0) as jpakai,FORMAT(SUM(r.uangair),0) as juangair,FORMAT(SUM(r.adm),0) as jadm,FORMAT(SUM(r.meter),0)as jmeter,FORMAT(SUM(r.meterai),0) as jmeterai,FORMAT(SUM((r.uangair + r.adm + r.meter + r.meterai)),0) as jtotal, format(ifnull((sum(r.uangair)/sum(r.pakai)),0),0)as avg_harga, format(ifnull((sum(r.pakai)/count(*)),0),0)as avg_plg
										FROM rekening1_awal as r
										INNER JOIN master AS m ON m.nomor=r.nomor
										WHERE $slipit (r.periode BETWEEN $periode AND $periode2) AND m.putus=0 AND ".$sql_pakai." AND ".$sql_bayar." GROUP BY m.gol ORDER BY m.gol
										;";
										...
										FROM rekening1_awal as r
										INNER JOIN master AS m ON m.nomor=r.nomor
										WHERE $slipit (r.periode BETWEEN $periode AND $periode2) AND m.putus=0 AND ".$sql_pakai." AND ".$sql_bayar."
										;";
										$qry0=mysql_query($sql_jml)or die("err: ".mysql_error());
										if($row0=mysql_fetch_row($qry0)){
											$jrek=$row0[0];
