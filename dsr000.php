...
?>
	...
	<tr>
		...
		<table width="850px" border="0" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF" align="center" class="wrapper"  style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:11px;box-shadow: 0 0 15px #999;background-color:#f0f0f0;" background="img/grids.gif">
			...
			<tr>
				...
				<td width="370">
					...
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					Tgl&nbsp;:&nbsp;&nbsp;
					<input type="text" size="9" maxlength="10" name="txtakhir" id="txtakhir" class="datepicker" value="<?php echo date('Y-m-d');?>" style="text-align:center;" />
                </td>
			...
			<tr>
				<td><input type="radio" name="r1" value="perupp" checked="checked">Cabang [UPP]</td>
				<td>
							...
							<?php
								if (isset($_POST["proses"])&& $_POST["proses"]=="Proses"){
									...
									if(isset($_POST["chkwm"])&&($key=$_POST['optkwm'])!=''){
										...
										$kwm=kondisi_wm($key);
										$judul.="<br>Kondiwi Water Meter : $key - $kwm";
									}
									...
									if(isset($_POST["rjrep"])&& $_POST["rjrep"]=="rjperpel"){
										...
										WHERE $slipit (r.periode BETWEEN $periode AND $periode2) AND m.putus=".$sambung." AND ((r.status_bayar=1 AND t.batal=0 AND t.tbayar<='$takhir')OR(r.status_bayar=0))
									...
									if(isset($_POST["rjrep"])&& $_POST["rjrep"]=="rjperpel666"){
										...
										$sql_jml="SELECT format(SUM(r.uangair),0),format(SUM(r.adm),0),format(SUM(r.meter),0),format(SUM(r.meterai),0),format(SUM(r.denda),0),format(SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)),0) FROM rekening1 AS r INNER JOIN master AS m ON m.nomor=r.nomor LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode WHERE $slipit (r.periode between $periode and $periode2) and r.status_bayar=0 and m.putus=".$sambung." ORDER BY m.nolama;";
									...
									if(isset($_POST["rjrep"])&& $_POST["rjrep"]=="rjdaftar"){
										...
										//```````````````````````````BOF OPTIONAL FOR FOOTER`````````````````````````````````
										$sql_jml="SELECT format(COUNT(r.periode),0),format(SUM(r.pakai),0),format(SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)),0) FROM rekening1 AS r INNER JOIN master AS m ON m.nomor=r.nomor LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode WHERE $slipit (r.periode between $periode and $periode2) and r.status_bayar=0 AND m.putus=".$sambung." AND ((r.status_bayar=1 AND t.batal=0 AND t.tbayar<='$takhir')OR(r.status_bayar=0));";
									...
									if(isset($_POST["rjrep"])&& $_POST["rjrep"]=="rjrekap"){
										...
										echo"<script type=\"text/javascript\">document.getElementById(\"jrep\").checked=\"checked\";</script>";	
										...
										$sql="SELECT DISTINCT m.gol,(select distinct t.keterangan from tarif AS t where t.gol=m.gol LIMIT 0,1)as ket,FORMAT(COUNT(*),0) as jml_rek,FORMAT(SUM(r.pakai),0) as jpakai,FORMAT(SUM(r.uangair),0) as juangair,FORMAT(SUM(r.adm),0) as jadm,FORMAT(SUM(r.meter),0)as jmeter,FORMAT(SUM(r.meterai),0) as jmeterai,FORMAT(SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)),0) as jtotal,FORMAT(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0)as avg_harga, FORMAT(IFNULL((SUM(r.pakai)/COUNT(*)),0),0)as avg_plg FROM rekening1 AS r INNER JOIN master AS m ON m.nomor=r.nomor LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode WHERE $slipit (r.periode between $periode and $periode2) AND m.putus=".$sambung." AND ((r.status_bayar=1 AND t.batal=0 AND t.tbayar<='$takhir')OR(r.status_bayar=0)) GROUP BY m.gol;";
										...
										//```````````````````````````BOF OPTIONAL FOR FOOTER`````````````````````````````````
										$sql_jml="SELECT format(count(*),0),format(SUM(r.pakai),0),format(SUM(r.uangair),0),format(SUM(r.adm),0),format(SUM(r.meter),0),format(SUM(r.meterai),0),format(SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)),0) FROM rekening1 AS r INNER JOIN master AS m ON m.nomor=r.nomor LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode WHERE $slipit (r.periode between $periode and $periode2) AND m.putus=".$sambung." AND ((r.status_bayar=1 AND t.batal=0 AND t.tbayar<='$takhir')OR(r.status_bayar=0)) ORDER BY m.gol;";
