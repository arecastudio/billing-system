...
?>
	...
	<tr>
		...
		<center><font face="Arial,sans-serif" size="4px" style="padding:2px 0;text-shadow:4px -4px 4px red;color:#fff;font-size:18px;"><b>Daftar Rekening Ditagihkan [DRD]</b></font></center>
		<table width="800px" border="0" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF" align="center" class="wrapper"  style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:11px;box-shadow: 0 0 15px #999;background:linear-gradient(#c0c0c0,#fff);" background="img/grids.gif">
			<form id="fr1" name="fr1" method="post" action="" onSubmit="">
			<tr>
				<td width="130px">Jenis Rek.</td>
				<td width="300px">					
					<input type="radio" name="rjenis" id="rjenis" value="rjtill" />DRD Awal &nbsp;&nbsp;
					<input type="radio" name="rjenis" id="rjenis" value="rjall" checked="checked" />DRD Berjalan
				</td>
			  	<td>Periode Rek.</td>
				<td>
							...
							<?php
								if (isset($_POST["proses"])&& $_POST["proses"]=="Proses"){
									...
									if(isset($_POST["chpergol"])&&($key=$_POST['optgoltarif'])!=''){
										$slipit=cek_slip($slipit)."r.gol='$key'";
									...
									if(isset($_POST["rjrep"])&& $_POST["rjrep"]=="rjdetail"){
										$judul="Daftar Rekening Ditagihkan ".$judul;
										$i=0;
										$theader="
										<tr align=\"center\" style=\"background:linear-gradient(#ffc,#fff);color:#000;font-weight:bold;\">
											...
											<td width=50>DKD</td>
											<td width=50>St<br>Lalu</td>
											<td width=50>St<br>Kini</td>
											<td width=50>Pakai</td>
											<td width=75>Uang<br>Air</td>
											<td width=60>Adm</td>
											<td width=60>Meter</td>
											<td width=60>Meterai</td>
										...
										/*$sql="
										SELECT r.nomor, m.nolama, SUBSTR(m.nama,1,25), m.dkd, r.standlalu, r.standkini, r.pakai, format(r.uangair,0), format(r.adm,0), format(r.meter,0), format(r.meterai,0), format(r.total,0) FROM rekening1 AS r
										INNER JOIN master AS m on m.nomor=r.nomor
										WHERE $slipit r.periode=$periode ORDER BY m.nolama
										;";*/
										
										$sql="
										SELECT r.nomor, r.nolama, SUBSTR(r.nama,1,25), r.dkd, r.standlalu, r.standkini, r.pakai, format(r.uangair,0), format(r.adm,0), format(r.meter,0), format(r.meterai,0), format(r.total,0) FROM rekening1 AS r
										INNER JOIN master AS m on m.nomor=r.nomor AND m.putus=0
										WHERE $slipit r.periode=$periode ORDER BY r.nolama,r.periode
										;";	
										
										$qry=mysql_query($sql)or die("err: ".mysql_error());
										echo "<tr align=\"center\" style=\"background:linear-gradient(#ffc,#fff);color:#000;font-weight:bold;\">
											<td>No</td>
											<td >Nomor</td>
											<td >NoLama</td>
											<td >Nama</td>
											...
											<td>Meterai</td>
											<td >Total</td>
										</tr>";//Cetak TR pertama sebagai header table
										...
										INNER JOIN master AS m ON m.nomor=r.nomor AND m.putus=0
										WHERE  $slipit (r.periode BETWEEN $periode AND $periode2) ORDER BY m.nolama;
										";
										...
										$tfooter="
										<tr align=\"center\" style=\"background-color:#fff;color:#000;font-size:11px;font-weight:bold;\">											
									...
									//=====================================BOF=REKAP==============================================
										...
										$judul="Ikhtisar Rekening Air ".$judul;
										$i=0;
										$theader="
										<tr align=\"center\" style=\"background:linear-gradient(#ffc,#fff);color:#000;font-weight:bold;\">
											<td width=30>No.</td>
											<td width=40>KODE</td>
											<td width=150>GOLONGAN TARIF</td>
											<td width=40>JML<br>REK.</td>
											<td width=40>PAKAI<br>AIR M<sup>3</sup></td>
											<td width=60>HARGA AIR<br>Rp.</td>
											<td width=50>ADM</td>
											<td width=50>METER</td>											
											<td width=55>METERAI</td>
										...
										$sql="SELECT distinct m.gol,(select distinct t.keterangan from tarif AS t where t.gol=m.gol)as ket,FORMAT(COUNT(*),0) as jml_rek,FORMAT(SUM(r.pakai),0) as jpakai,FORMAT(SUM(r.uangair),0) as juangair,FORMAT(SUM(r.adm),0) as jadm,FORMAT(SUM(r.meter),0)as jmeter,FORMAT(SUM(r.meterai),0) as jmeterai,FORMAT(SUM(r.total),0) as jtotal, format(ifnull((sum(r.uangair)/sum(r.pakai)),0),0)as avg_harga, format(ifnull((sum(r.pakai)/count(*)),0),0)as avg_plg FROM rekening1 AS r INNER JOIN master AS m ON m.nomor=r.nomor AND m.putus=0 WHERE $slipit (r.periode BETWEEN $periode AND $periode2) GROUP BY m.gol ORDER BY m.gol;";
										...
										";//Cetak TR pertama sebagai header table
										while($row=mysql_fetch_row($qry)){
											$i++;
											echo"
											<tr>
										...
										//```````````````````````````BOF OPTIONAL FOR FOOTER`````````````````````````````````
										//$sql_jml="SELECT format(count(*),0),format(SUM(r.pakai),0),format(SUM(r.uangair),0),format(SUM(r.adm),0),format(SUM(r.meter),0),format(SUM(r.meterai),0),format(SUM(r.total),0) FROM rekening1 r,master m WHERE $slipit m.nomor=r.nomor AND (r.periode BETWEEN $periode AND $periode2);";
										$sql_jml="SELECT FORMAT(COUNT(*),0),FORMAT(SUM(r.pakai),0),FORMAT(SUM(r.uangair),0),FORMAT(SUM(r.adm),0),FORMAT(SUM(r.meter),0),FORMAT(SUM(r.meterai),0),FORMAT(SUM(r.total),0) FROM rekening1 as r WHERE $slipit (r.periode BETWEEN $periode AND $periode2);";
										$qry0=mysql_query($sql_jml)or die("err: ".mysql_error());
										if($row0=mysql_fetch_row($qry0)){
											$jrek=$row0[0];
											$jpakai=$row0[1];
