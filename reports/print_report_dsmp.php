<?php
	...
	while($row=mysql_fetch_row($qry)){
		...
		if($re<=1){
			...
			$hal++;
			$cetak8 .= "</table>".$footer1."</page><page><div align=\"right\"></div>".$header1."<table align=\"left\" height=\"65px\"border=\"1\"  style=\"border-style:dashed;border-collapse:collapse;background:#fff;font:Arial;font-size:12px;border-spacing:2px;padding:2px;width:1024px;\">".$theader;
			$re=0;
			//++$hal;
		}
		$cetak8 .="<tr height=\"65px\"><td align=\"center\" >".$j."</td>";
		for($i=0;$i<$nmr;$i++){
			//$cetak8 .="<td align=\"$alg[$i]\">".trim($row[$i])."</td>";
			if($i==2||$i==3){
				$cetak8 .="<td height=\"65px\" align=\"$alg[$i]\">".ucwords(trim($row[$i]))."</td>";
			}else{
				$cetak8 .="<td height=\"65px\" align=\"$alg[$i]\">".trim($row[$i])."</td>";
			}
		}
		$cetak8 .="</tr>";					
	}
