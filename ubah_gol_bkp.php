			...
			<form id="fr1" name="fr1" method="post" action="" onSubmit="return suvalid(this);">
				...
				<tr>
					...
					<td colspan="2"><input type="hidden" id="txhnola" name="txhnola">
						<?php
						echo"<input type=\"radio\" name=\"r1\" id=\"r1\" value=\"auto\" checked> Automatic<br/>";
						if($dapat==1){
						...
						<br/><br/><br/><br/>
						<?php
						echo"<input type=\"radio\" name=\"r1\" id=\"r2\" value=\"manu\" > Manual <i style=\"color:#f00;\">[khusus pel. UPP Genyem atau Gol A5 - Rmh Tgg TNI dll]</i><br/>";
						?>
						
						<select name="optgol2" id="optgol2" style=\"font-weight:bold;color:#00f;background-color:#fff;\">
							<!--option value="" selected="selected">--Pilih--</option-->
							<?php
								$sql=mysql_query("SELECT DISTINCT gol,keterangan FROM golongan ORDER BY gol ASC;");
								while($row=mysql_fetch_row($sql)){
		...
		<?php			
			...
			if(isset($_POST['save'])){
				if($_POST['r1']=='auto'){
					...
					//echo"nomor: $snomor \n nolama: $slama - $kode \n gol: $gol";
					$sql="INSERT INTO ubah_gol(nomor,tgl,jam,nola1,nola2,gol1,gol2,operator,loket)
					VALUES('$snomor','$tgl','$jam','$nolaawal','".$slama.$kode."','$golawal','$gol','$operator','$nloket')
					ON DUPLICATE KEY UPDATE jam='$jam',tgl='$tgl',nola1='$nolaawal',nola2='".$slama.$kode."',gol1='$golawal',gol2='$gol',operator='$operator',loket='$nloket'
					;";
					mysql_query($sql)or die(mysql_error());
					...
					//echo"nomor: $snomor \n nolama: $slama \n gol: $gol";
					$sql="INSERT INTO ubah_gol(nomor,tgl,jam,nola1,nola2,gol1,gol2,operator,loket)
					VALUES('$snomor','$tgl','$jam','$nolaawal','$slama','$golawal','$gol','$operator','$nloket')
					ON DUPLICATE KEY UPDATE jam='$jam',tgl='$tgl',nola1='$nolaawal',nola2='$slama',gol1='$golawal',gol2='$gol',operator='$operator',loket='$nloket'
					;";
					mysql_query($sql)or die(mysql_error());
				}
