...
$jam=date('H:m:i');
/*
//$qry="insert into dsmp_new(nomor,periode) select nomor,kode_blok from master where kode_blok='$peri' and putus='0' and not exists(select * from dsmp_new where periode='$plalu');";
$qry="insert ignore into dsmp_new(nomor,periode) select nomor,periode_t from master where putus='0';";
$sql=mysql_query($qry) or die(mysql_error());
//if($ok=mysql_fetch_row($sql)){echo"sip";}

//$qry="update dsmp_new set angka=0 where periode is null";
//$sql=mysql_query($qry) or die(mysql_error());
//mysql_fetch_row($sql);

$qry="update dsmp_new set angka=0,periode=0 where (periode=0 OR periode IS NULL)";
$sql=mysql_query($qry) or die(mysql_error());
//mysql_fetch_row($sql);

$qry="update ignore dsmp_new set periode='$plalu' where periode=0";
$sql=mysql_query($qry) or die(mysql_error());
//mysql_fetch_row($sql);

$qry="delete from dsmp_new where periode=0";
$sql=mysql_query($qry) or die(mysql_error());
...
//$qry="insert into dsmp_new(nomor,periode) select nomor,kode_blok from master where kode_blok='$peri' and putus='0' and not exists(select * from dsmp_new where periode='$pkini');";
$qry="insert into dsmp_new(nomor,periode) select nomor,periode_t from master where putus='0';";
$sql=mysql_query($qry) or die(mysql_error());
//if($ok=mysql_fetch_row($sql)){echo"sip";}
...
//$qry="update dsmp_new set angka=0 where periode=0";
$qry="update dsmp_new set angka=0,periode=0 where (periode=0 OR periode IS NULL)";
$sql=mysql_query($qry) or die(mysql_error());
//mysql_fetch_row($sql);

$qry="update ignore dsmp_new set periode='$pkini' where periode=0";
$sql=mysql_query($qry) or die(mysql_error());
//mysql_fetch_row($sql);
...
function simpan($idata){	
	...
	for ($j=1;$j<=$idata;$j++){ //untuk nomor pake input type hiden untuk tampung,,sip..!!!!!!!
		//$query="update dsmp_new set angka='".now."$i' where nomor='$tmp[$j]' and periode='$pkini'";
		$query="UPDATE dsmp_new AS a,dsmp_new AS b SET b.angka='".now."$i',a.angka='".lst."$i' where b.nomor='$tmp[$j]' AND a.nomor=b.nomor AND b.periode=$pkini AND a.periode=$plalu";
...
if (isset($_POST['save'])){
	...
	while($j<$jum){
		...
		$lalu[$j]=strip_tags($_POST['lst'.$j]);
		$smbr[$j]=trim($_POST['smbr'.$j]);
		//$angl[$j]=strip_tags($_POST['lst'.$j]);
		$nmr[$j]=strip_tags($_POST['num'.$j]);
		//$sql="update dsmp_new set angka=$ang[$j] where nomor='$nmr[$j]' and periode='$pkini';";
		//mysql_query($sql) or die(mysql_error());
		$kwm0[$j]=strip_tags($_POST['kwm'.$j]);
		$kwm_lama[$j]=strip_tags($_POST['kwml'.$j]);//kondisi water meter di master(sebelum diubah)
	}
	$j=0;
	while($j<$jum){
		$j++;
		...
		$sql="update dsmp_new set angka=$ang[$j] where nomor='$nmr[$j]' and periode=$pkini;";
		mysql_query($sql) or die(mysql_error());
		//$sql2="update dsmp_new set angka=$angl[$j] where nomor='$nmr[$j]' and periode=$plalu;";
	...
	while($j<$jum){
		...
		//$ang[$j]=strip_tags($_POST['now'.$j]);
		//$nmr[$j]=strip_tags($_POST['num'.$j]);
		$sql="update dsmp_new set angka=$lalu[$j] where nomor='$nmr[$j]' and periode=$plalu;";
		mysql_query($sql) or die(mysql_error());
		//$sql2="update dsmp_new set angka=$angl[$j] where nomor='$nmr[$j]' and periode=$plalu;";
		//mysql_query($sql2) or die(mysql_error());
	}
	...
	while($j<$jum){
		$j++;
		//$sql="update master set kondisi_meter=$kwm0[$j] where nomor='$nmr[$j]';";
		switch($kwm0[$j]){
			case"1":
				$sql="update master set kondisi_meter=$kwm0[$j] where nomor='$nmr[$j]';";
				break;
			case"2":
				$sql="UPDATE master AS m INNER JOIN dsmp_new AS a ON a.nomor=m.nomor AND a.periode=$plalu INNER JOIN dsmp_new AS b ON b.nomor=m.nomor AND b.periode=$pkini SET m.kondisi_meter=$kwm0[$j],a.angka=0,b.angka=0 WHERE m.nomor='$nmr[$j]';";
				break;
			case"3":
				$sql="UPDATE master AS m INNER JOIN dsmp_new AS a ON a.nomor=m.nomor AND a.periode=$plalu SET m.kondisi_meter=$kwm0[$j],a.angka=0 WHERE m.nomor='$nmr[$j]';";
				break;
			case"4":
				$sql="UPDATE master AS m INNER JOIN dsmp_new AS a ON a.nomor=m.nomor AND a.periode=$plalu SET m.kondisi_meter=$kwm0[$j],a.angka=0 WHERE m.nomor='$nmr[$j]';";
				break;
		...
		$q23355="IF NOT EXISTS (SELECT m.nomor FROM master AS m WHERE m.kondisi_meter='$kwm0[$j]' AND m.nomor='$nmr[$j]')
		INSERT INTO ubah_kwm(nomor,tgl,jam,lama,baru,operator,loket,sumber)
		VALUES('$nmr[$j]','$tgl','$jam'(SELECT DISTINCT n.kondisi_meter FROM master AS n WHERE n.nomor='$nmr[$j]' LIMIT 1),'$kwm0[$j]','$operator','$nloket',$sumber)
		ON DUPLICATE KEY UPDATE baru='$kwm0[$j]',jam='$jam',operator='$operator',loket='$nloket'
		;";
		
		$q2="
		INSERT INTO ubah_kwm(nomor,tgl,jam,lama,baru,operator,loket,sumber)
		SELECT * FROM (SELECT '$nmr[$j]','$tgl','$jam','$kwm_lama[$j]','$kwm0[$j]','$operator','$nloket','$smbr[$j]') AS nilai		
		ON DUPLICATE KEY UPDATE baru='$kwm0[$j]',jam='$jam',operator='$operator',loket='$nloket',sumber='$smbr[$j]'
		;";
		
		if($kwm_lama[$j]!=$kwm0[$j])mysql_query($q2) or die("<div style=\"color:yellow;\">".mysql_error()."</div>");
		
	}
	$j=0;
	
	//for($k=1;k<=$jum;$k++){
	//	$sql="update dsmp_new set angka=$ang[$k] where nomor='$nmr[$k]' and periode='$pkini';";
	//	mysql_query($sql) or die(mysql_error());
	//}
	
	//mysql_query("update billing.dsmp_new set angka=2233 where nomor='000001' and periode='201406';") or die(mysql_error());
	...
	//for($i=0;$i<=$jl;$i++){
		...
		$pr1=$row[3];
		if($kn1=='1'){
			$q1="INSERT IGNORE INTO dsmp_new(nomor,periode,angka)VALUES('$nr1',$plalu,0);";
			$q2="INSERT IGNORE INTO dsmp_new(nomor,periode,angka)VALUES('$nr1',$pkini,0);";
		}else if($kn1=='2'){
			$q1="INSERT INTO dsmp_new(nomor,periode,angka)VALUES('$nr1',$plalu,0) ON DUPLICATE KEY UPDATE angka=0;";
			$q2="INSERT INTO dsmp_new(nomor,periode,angka)VALUES('$nr1',$pkini,0) ON DUPLICATE KEY UPDATE angka=0;";
		}else{
			$q1="INSERT INTO dsmp_new(nomor,periode,angka)VALUES('$nr1',$plalu,0) ON DUPLICATE KEY UPDATE angka=0;";
			$q2="INSERT INTO dsmp_new(nomor,periode,angka)VALUES('$nr1',$pkini,$pk1) ON DUPLICATE KEY UPDATE angka=$pk1;";
			//echo"tigaempat ".++$i;
		}
			...
			<tr>
				...
				<td bgcolor="#eee">
					...
					<select id="optkolektif" name="optkolektif" >
						<option value="col" selected="selected">Semua Kolektif</option>
						<?php
					...
					<input type="text" name="txnomor" id="txnomor" size="13" maxlength="13" onKeyPress="return isNumb(event);"> 
							</td>
				<td bgcolor="#eee">
					Auto-sort by:<br />
					...
					<input type="radio" name="r4" id="sort_rute" value="rute">Rute Baca<br/>
					<input type="radio" name="r4" id="sort_pakai" value="pakai">Pakai</td>
				<td bgcolor="#eee">
					Tampilkan:<br />
                    ...
                    <input type="radio" name="r5" id="kosong2" value="kosong2">Kosong
               	</td>
			</tr>
			<tr>
				...
				<td colspan="4" align="center" bgcolor="#0f0">
					...
					<table width="100%" border="1" bordercolor="white" style="border-collapse:collapse;background:#ffc;font-family:calibri;font-size:11px;">
						...
						<tr align="center" style="color:#fff;background:#99ccff;font-size:11px;font-family:calibri">
							...
							<td width="30px"><?php echo getBulan(date("m")-1);?></td>
							<td width="30px"><?php echo getBulan(date("m"));?></td>
						...
						<?php
							...
							//$query=mysql_query("select master.nomor,master.nolama,master.nama,dsmp_new.angka from master,dsmp_new where master.putus='0' and master.kondisi_meter='1' and master.kode_blok='0101' and dsmp_new.nomor=master.nomor and dsmp_new.periode='$plalu' order by master.nolama;") or die(mysql_error());
						...
						if(isset($_POST["proses"])){
							...
							switch($nmr=$_POST['r3']){
								...
								case"dkd":
									...
									break;
								case"kolektif":
									if(($key=$_POST['optkolektif']) !="col")$slipit=cek_slip($slipit)."k.kode_kolektif='$key'";
									if(($key=$_POST['optkolektif']) =="col")$slipit=cek_slip($slipit)."k.kode_kolektif<>'$key'";
							...
							switch($sordby){
								...
								case"rute":
									...
									break;
								case"pakai":
									$slipit .=" ORDER BY (b.angka-a.angka)";
									echo"<script type=\"text/javascript\">document.getElementById(\"sort_pakai\").checked=\"checked\";</script>";
									break;
							}				
														
							//$sql="SELECT DISTINCT m.nomor,m.nolama,m.nama,a.angka,b.angka,m.dkd,m.kondisi_meter FROM master AS m,dsmp_new AS a,dsmp_new AS b $slipit ;";
							//echo $sql;
							$sql="
							...
							while($row=mysql_fetch_row($query)){
								...
								echo"<td>".$row[2]."</td>";
								/*if($row[6]==1){*/
                                	echo"<td><input type=\"text\" style=\"text-align:right;\" name=\"lst"."$i\" value=\"$a1\" size=\"6\" /></td>";
								...
								for($k=1;$k<5;$k++){
									if($k!=$kwm) echo"<option value=\"$k\">$k</option>";
								}
								echo"
									</select>
								</td>
								";
								echo"<input name=\"kwml".$i."\" type=\"hidden\" value=\"$kwm\">";//kondisi water meter pada master
								echo"<td>
<select id=\"smbr"."$i\" name=\"smbr"."$i\">
