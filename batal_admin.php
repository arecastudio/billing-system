...
kunci_halaman("util_pembatalan_transaksi",$secret,$operator);
	...
	<tr>
		...
		<table width="1045px" border="0" cellpadding="1" cellspacing="1" align="center" class="wrapper"  style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:11px;box-shadow: 0 0 15px #999;background-color:#f0f0f0;" background="img/grids.gif">
          <form id="fr1" name="fr1" method="post" action="" onSubmit="">
            ...
            <tr>
            	<td width="130px">Tanggal Bayar</td>
                <td width="500px" align="left">
                	<input type="text" size="8px" name="txtgl" id="txtgl" readonly class="datepicker" value="<?php echo date('Y-m-d');?>" />
                </td>
							...
							<?php
								if (isset($_POST["proses"])&& $_POST["proses"]=="Tampilkan!" && $dapat==1){
									...
									if(isset($_POST["chjlok"])&&($key=$_POST['optjlok'])!=''){
										//$slipit=cek_slip($slipit)."(t.loket='$key' OR r.loket='$key')";
										$sisip="(t.loket='$key' OR r.loket='$key') AND ";
										//$judul.="<br>Loket : $key";
									...
									WHERE $sisip r.nomor='$nmr' AND r.status_bayar=1 AND (r.tbayar='".trim($_POST['txtgl'])."' OR t.tbayar='".trim($_POST['txtgl'])."')
...
if (isset($_POST["batalkan"])){	
	...
	$sql="SELECT DISTINCT r.nomor,r.periode,(r.uangair + r.adm + r.meter + r.meterai + r.denda) FROM rekening1 AS r LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode WHERE r.status_bayar=1 AND (r.tbayar='".$_POST['txtgl']."' OR t.tbayar='".$_POST['txtgl']."') AND r.nomor='$nomor';";
	$query=mysql_query($sql)or die(mysql_error());
	while($row=mysql_fetch_row($query)){
		$prd=trim($row[1]);
		$total=trim($row[2]);// AND tbayar='".$_POST['txtgl']."'
		$nmr1=trim($row[0]);		//AND tbayar='".$_POST['txtgl']."'  AND batal=0
		if(isset($_POST[$prd])){
			mysql_query("UPDATE rekening1 SET status_bayar=0,tbayar='0000-00-00',loket='',denda=0 WHERE nomor='$nomor' AND periode=$prd;")or die(mysql_error());
			mysql_query("DELETE FROM transaksi WHERE nomor='$nomor' AND periode=$prd;")or die(mysql_error());
