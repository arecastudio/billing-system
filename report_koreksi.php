...
kunci_halaman("util_report_koreksi",$secret,$operator);
$lembar=0;
$pakai=0;
$uangair=0;
$adm=0;
...
?>
		...
		<table width="920px" border="0" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF" align="center" class="wrapper"  style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:11px;box-shadow: 0 0 15px #999;background-color:#f0f0f0;" background="file:///C|/Users/Asus/AppData/Local/Temp/scp46941/var/www/html/billing/img/grids.gif">
          <form id="fr1" name="fr1" method="post" action="" onsubmit="">
            <tr style="font:calibri;font-size:18px;font-weight:bold;color:#fff;background:linear-gradient(#666,#004);">
              <td colspan="4"><center>
                <font face="Arial,sans-serif" size="4px" style="padding:2px 0;text-shadow:4px -4px 4px red;color:#fff;font-size:19px;"><b>.::  Laporan Koreksi Rekening Pelanggan  ::.</b></font>
              </center></td>
            </tr>
            <tr>
 ...
 <?php
						$sql=mysql_query("select distinct periode from his_koreksi_rek order by periode ASC");
                ...
                <select name="optperiode2" id="optperiode2">
                  	<!--option value="" selected="selected">--Pilih--</option-->
                  	<?php
						$sql=mysql_query("select distinct periode from his_koreksi_rek order by periode desc");
						//$sql=mysql_query("select distinct max(periode) from rekening1;");
						//$sql=mysql_query("select (201408-0) as prd;");
						while($row=mysql_fetch_row($sql)){
							//$i=0;							
					...
					?>
                </select> <input type="radio" name="rnama"  value="perkor" id="rkor"/>
Detal Koreksi Rekening</td>
              <td>&nbsp;</td>
              <td><input type="radio" name="rnama"  value="perrekap" id="rrekap"/>
              Rekap</td>
            </tr>
            <tr>
            ...
            <tr>
              <td><input type="checkbox" name="chpergol"  value="pergol" id="chpergol"/>
Per Gol.</td>
              <td><select name="optgoltarif" id="optgol" >
                  <!--option value="" selected="selected">--Pilih--</option-->
                  <?php
							...
							<?php
								if (isset($_POST["proses"])&& $_POST["proses"]=="Proses"){
									...
									//switch($radio1){
										...
										echo"<script type=\"text/javascript\">document.getElementById(\"chpergol\").checked=\"checked\";</script>";
										echo"<script type=\"text/javascript\">document.getElementById(\"optgol\").value=\"$key\";</script>";
										$judul.="<br>Golongan Tarif : $key";
									}
										//DETAIL KOREKSI REKENING	
									if(isset($_POST["rnama"])&& $_POST["rnama"]=="perkor"){
										...
										LEFT OUTER JOIN transaksi AS z ON z.nomor=k.nomor
										WHERE (k.tkorek BETWEEN '$tawal' AND '$takhir') AND (k.periode BETWEEN $periode AND $periode2) and $slipit
										ORDER BY k.nomor, k.tkorek ASC
										;"; 
									...
									//REKAP KOREKSI REKENING	
									if(isset($_POST["rnama"])&& $_POST["rnama"]=="perrekap"){
										...
										m.nolama,
										COUNT(k.periode),
										SUM(k.standlalu) as standlalu,
										SUM(k.standkini) as standkini,
										sum(k.pakai) as pakai,
										format(SUM(k.uangair + k.adm + k.meter + k.denda + k.meterai),0) as total,
										k.tkorek,
										k.loket,
										k.operator,
										SUM(k.standlalu_awal),
										SUM(k.standkini_awal),
										SUM(k.pakai_awal) as pakai_awal,
										format(SUM(k.uangair_awal + k.adm_awal + k.meter_awal + k.denda_awal + k.meterai_awal),0) as total_awal,
										format(pakai_awal-pakai,0),
										format(total_awal-total,0),
										k.tdisposisi,
										k.alasan,
										m.nama
										FROM his_koreksi_rek AS k
										...
										while($row=mysql_fetch_row($hasil)){
											...
											<tr>
												...
												<td align=right>$row[15]</td>
												<td align=right>$row[17]</td>
											...
											$i++;
										  	echo"
										  	<tr align=\"center\" style=\"background-color:#ddd;color:#000;font-weight:bold;\">											
											<td style=\"border:1px solid;\" colspan=\"9\" height=\"20\">TOTAL KOREKSI REKENING</td>
											...
											";	}							
										echo"<script type=\"text/javascript\">document.getElementById(\"rrekap\").checked=\"checked\";</script>";
...
//									if(isset($_POST["rnama"])!="pergol"){
								/*	if(isset($_POST["rnama"])&& $_POST["rnama"]=="perdsmp"){
									...
									//=====================================BOF=DETAIL==============================================
								/*	if(isset($_POST["rjreppp"])&& $_POST["rjreppp"]=="rppjdetail"){
								...
								/*		$_SESSION['orientasi']='L';
										...
										*/
									//=====================================BOF=REKAP==============================================
							/*		if(isset($_POST["rjreppp"])&& $_POST["rjrppep"]=="rjreppkap"){
