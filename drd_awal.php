	...
	<tr>
		...
		<table width="1024px" border="0" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF" align="center" class="wrapper"  style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:11px;box-shadow: 0 0 15px #999;background:linear-gradient(#c0c0c0,#fff);" background="img/grids.gif">
			...
			<tr style="font:calibri;font-size:18px;font-weight:bold;color:#fff;background:linear-gradient(#666,#004);text-align:justify;">
            	<td colspan="4" align="center"><center><font face="Arial,sans-serif" size="4px" style="padding:2px 0;text-shadow:4px -4px 4px red;color:#fff;font-size:19px;"><b>DRD Awal [Sebelum Koreksi]</b></font></center></td>
            </tr>
            <tr>
				<td width="130px">Jenis Rek.</td>
				<td width="300px">					
					<input type="radio" name="rjenis" id="rjenis" value="rjtill" checked="checked" />DRD Awal &nbsp;&nbsp;
					<input type="radio" name="rjenis" id="rjenis" value="rjall" disabled />DRD Berjalan				</td>
			  	<td>Periode Rek.</td>
				<td>
			...
			<tr>
				...
				<td><select name="optpakai88" id="optpakai88">
                  <option value="0">Semua</option>
                  <option value="1">0 sampai 10</option>
                  <option value="2">11 sampai 20</option>
                  <option value="3">lebih dari 20</option>
                  <option value="4">lebih dari 100</option>
                  <option value="6">200 atau lebih</option>
                  <option value="5">lebih dari 1000</option>
                </select></td>
                </tr>
            <tr>
            <td>&nbsp;
            ...
            <td>
                ...
                <td align="right">Status Bayar
            </td>
            <td><select name="optbayar" id="optbayar">
              <option value="0">Semua</option>
              <option value="1">Lunas</option>
              <option value="2">Tunggak</option>
            </select>            </td>
			</tr>
			<tr height="50px">
			...
			<tr>
				<td width="100%" align="center">
					<div style="height:300px;overflow:auto">
						<table width="100%" border="1" id="table-hasil" cellpadding="1" cellspacing="1" align="center" bgcolor="white" style="border-collapse:collapse;background:#ffe;color:#000;font-family:calibri;font-size:12px;">
...
#echo "44444444".$judul;
switch($pakai){
	case"1":echo"<script type=\"text/javascript\">document.getElementById(\"optpakai88\").value=\"$pakai\"</script>";$var_pakai="0 - 10 m3";$sql_pakai="(r.pakai BETWEEN 0 AND 10)";break;
	case"2":echo"<script type=\"text/javascript\">document.getElementById(\"optpakai88\").value=\"$pakai\"</script>";$var_pakai="11 s/d 20 m3";$sql_pakai="(r.pakai BETWEEN 11 AND 20)";break;
	case"3":echo"<script type=\"text/javascript\">document.getElementById(\"optpakai88\").value=\"$pakai\"</script>";$var_pakai="Lebih dari 20 m3";$sql_pakai="(r.pakai>20)";break;
	case"4":echo"<script type=\"text/javascript\">document.getElementById(\"optpakai88\").value=\"$pakai\"</script>";$var_pakai="Lebih dari 100 m3";$sql_pakai="(r.pakai>100)";break;
	case"5":echo"<script type=\"text/javascript\">document.getElementById(\"optpakai88\").value=\"$pakai\"</script>";$var_pakai="Lebih dari 1000 m3";$sql_pakai="(r.pakai>1000)";break;
	case"6":echo"<script type=\"text/javascript\">document.getElementById(\"optpakai88\").value=\"$pakai\"</script>";$var_pakai="200 m3 ke atas";$sql_pakai="(r.pakai>=200)";break;
	default:echo"<script type=\"text/javascript\">document.getElementById(\"optpakai88\").value=\"$pakai\"</script>";break;
}

$bayar=$_POST["optbayar"];
									$var_bayar="Semua Transaksi";
									//$judul.="- Periode [$periode s/d $periode2]";
									$sql_bayar="(r.status_bayar>=0)";
									
#echo "44444444".$judul;
switch($bayar){
	case"1":echo"<script type=\"text/javascript\">document.getElementById(\"optbayar\").value=\"$bayar\"</script>";$var_bayar="Lunas";$sql_bayar="(r.status_bayar=1)";$judul.=" (Stat. LNS)";break;
	case"2":echo"<script type=\"text/javascript\">document.getElementById(\"optbayar\").value=\"$bayar\"</script>";$var_bayar="Tunggak";$sql_bayar="(r.status_bayar=0)";$judul.=" (Stat. TGK)";break;
	default:echo"<script type=\"text/javascript\">document.getElementById(\"optbayar\").value=\"$bayar\"</script>";break;
}
									...
									switch($radio1){
										...
										case"perwil":
											if(($key=$_POST["optwilayah"])!=""){
												$slipit=cek_slip($slipit)."r.kode_wilayah='$key'";
										...
										case"perblok":
											if(($key=$_POST["optblok"])!=""){
												$slipit=cek_slip($slipit)."r.kode_blok='$key'";
										...
										case"perdkd":
											if(($key=$_POST["optdkd"])!=""){
												$slipit=cek_slip($slipit)."r.dkd='$key'";
									...
									if(isset($_POST["chjpel"])&&($key=$_POST['optjpel'])!=''){
										...
										if ($key>'1') {
											$slipit=cek_slip($slipit)."(r.nomor in (select distinct d.nomor from pel_kolektif_d as d inner join pel_kolektif as p on p.kode_kolektif=d.kode_kolektif where p.kode_jenis_pel='$key'))";
										}else{
											$slipit=cek_slip($slipit)."(r.nomor in (select distinct nomor from master where nomor not in(select distinct d.nomor from pel_kolektif_d as d inner join pel_kolektif as p on p.kode_kolektif=d.kode_kolektif where p.kode_jenis_pel>'1')))";
										}
									...
									if(isset($_POST["rjrep"])&& $_POST["rjrep"]=="rjdetail"){
										...
										//$sql="SELECT r.nomor,m.nolama,m.nama,m.cabang,m.kode_wilayah,m.dkd,r.standlalu,r.standkini,r.pakai,format(r.uangair,0),format(r.adm,0),format(r.meter,0),format(r.meterai,0),format((r.uangair + r.adm + r.meter + r.meterai + r.denda),0) FROM rekening1_awal AS r, master AS m WHERE $slipit r.nomor=m.nomor and r.periode=$periode ORDER BY m.nolama;";
										/*$sql="
										SELECT r.nomor, m.nolama, SUBSTR(m.nama,1,25), m.dkd, r.standlalu, r.standkini, r.pakai, format(r.uangair,0), format(r.adm,0), format(r.meter,0), format(r.meterai,0), format((r.uangair + r.adm + r.meter + r.meterai + r.denda),0) FROM rekening1_awal AS r
										...
										$sql="
										SELECT DISTINCT r.nomor, r.nolama, SUBSTR(lcase(r.nama),1,20), r.dkd, r.standlalu, r.standkini, r.pakai, format(r.uangair,0), format(r.adm,0), format(r.meter,0), format(r.meterai,0), format((r.uangair + r.adm + r.meter + r.meterai ),0),r.kondisi_meter,IF(r.status_bayar=1,'LNS','TGK')
										FROM rekening1_awal AS r
										WHERE $slipit (r.periode BETWEEN $periode AND $periode2) AND ".$sql_pakai." AND ".$sql_bayar."
										ORDER BY r.nolama,r.periode
										;";	
										...
										//$sql_jml="SELECT format(SUM(r.pakai),0),format(SUM(r.uangair),0),format(SUM(r.adm),0),format(SUM(r.meter),0),format(SUM(r.meterai),0),format(SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)),0) FROM rekening1_awal r, master m WHERE $slipit r.nomor=m.nomor and r.periode=$periode and m.putus=0 ORDER BY m.nolama;";
										$sql_jml="
										SELECT format(SUM(r.pakai),0), format(SUM(r.uangair),0), format(SUM(r.adm),0), format(SUM(r.meter),0), format(SUM(r.meterai),0), format(SUM((r.uangair + r.adm + r.meter + r.meterai )),0) FROM rekening1_awal AS r
										WHERE  $slipit (r.periode BETWEEN $periode AND $periode2) AND ".$sql_pakai." AND ".$sql_bayar." ORDER BY r.nolama;
										";
									...
									//=====================================BOF=REKAP==============================================
										...
										if($slipit!="")$slipit.=" AND ";
										//$sql="SELECT distinct m.gol, FROM rekening1_awal r, master m WHERE $slipit r.nomor=m.nomor and r.periode=$periode and m.putus=0 ORDER BY m.nolama;";
										$sql="
										SELECT DISTINCT r.gol,(select distinct t.keterangan from tarif t where t.gol=r.gol)as ket,FORMAT(COUNT(*),0) as jml_rek,FORMAT(SUM(r.pakai),0) as jpakai,FORMAT(SUM(r.uangair),0) as juangair,FORMAT(SUM(r.adm),0) as jadm,FORMAT(SUM(r.meter),0)as jmeter,FORMAT(SUM(r.meterai),0) as jmeterai,FORMAT(SUM((r.uangair + r.adm + r.meter + r.meterai)),0) as jtotal, format(ifnull((sum(r.uangair)/sum(r.pakai)),0),0)as avg_harga, format(ifnull((sum(r.pakai)/count(*)),0),0)as avg_plg
										FROM rekening1_awal as r
										WHERE $slipit (r.periode BETWEEN $periode AND $periode2) AND ".$sql_pakai." AND ".$sql_bayar." GROUP BY r.gol ORDER BY r.gol
										;";
										...
										//```````````````````````````BOF OPTIONAL FOR FOOTER`````````````````````````````````
										//$sql_jml="SELECT format(count(*),0),format(SUM(r.pakai),0),format(SUM(r.uangair),0),format(SUM(r.adm),0),format(SUM(r.meter),0),format(SUM(r.meterai),0),format(SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)),0) FROM rekening1_awal r,master m WHERE $slipit m.nomor=r.nomor AND (r.periode BETWEEN $periode AND $periode2);";
										$sql_jml="
										...
										<tr width=\"100%\">
											<td width=\"300\" align=\"center\">
												...
												Manager Keuangan<br>
												<br><br><br><br><br>
											        ABDULLAH MANSUR, S.E<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
											</td>
											<td width=\"300\" align=\"center\">
											<!--	Diperiksa oleh,<br>
												Ka.SubBid Pengelola Data Rekening<br><br><br><br><br><br>
												SOETRISNO, S.Kom<br>
												&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  -->
											</td>
											<td width=\"300\" align=\"center\">
												Dibuat oleh,<br>
												Plt. Asmen Pengelola Data Rekening<br><br><br><br><br><br>
			...
			<td align="left" style="font:Verdana, Arial, Helvetica, sans-serif;font-size:9px;background:linear-gradient(#fff,#c0c0c0);">
                ...
                <input type="button" name="export" value="Export to Excel" onClick ="$('#table-hasil').tableExport({type:'excel',escape:'false',htmlContent:'true'});" style="color:#00f;font-size:16px;font-weight:bold;padding:0px;"/>
