							...
							<?php
								if (isset($_POST["proses"])&& $_POST["proses"]=="Proses"){
									...
									echo"<script type=\"text/javascript\">document.getElementById(\"txtakhir\").value=\"$key2\";</script>";
										...
										WHERE $slipit (r.periode BETWEEN $periode AND $periode2) AND ((r.status_bayar=1 AND t.batal=0 AND t.tbayar>'$takhir')OR(r.status_bayar=0))ORDER BY m.nolama,r.periode
										;";
									...
									echo"<script type=\"text/javascript\">document.getElementById(\"txtakhir\").value=\"$key2\";</script>";
										...
										INNER JOIN master AS m ON m.nomor=r.nomor
										LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode
										WHERE $slipit (r.periode BETWEEN $periode AND $periode2) AND ((r.status_bayar=1 AND t.batal=0 AND t.tbayar>'$takhir') OR(r.status_bayar=0))ORDER BY m.nolama;";																		
										...
										$sql_jml="SELECT format(SUM(r.uangair),0),format(SUM(r.adm),0),format(SUM(r.meter),0),format(SUM(r.meterai),0),format(SUM(r.denda),0),format(SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)),0) 
										FROM rekening1 AS r
										INNER JOIN master AS m ON m.nomor=r.nomor
										LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode
										WHERE $slipit r.nomor=m.nomor and (r.periode between $periode and $periode2) AND ((r.status_bayar=1 AND t.batal=0 AND t.tbayar>'$takhir')OR(r.status_bayar=0))ORDER BY m.nolama;";
									...
									echo"<script type=\"text/javascript\">document.getElementById(\"txtakhir\").value=\"$key2\";</script>";
										...
										FROM rekening1 AS r
										INNER JOIN master AS m ON r.nomor=m.nomor
										LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode
										WHERE $slipit r.nomor=m.nomor and (r.periode BETWEEN $periode AND $periode2) AND ((r.status_bayar=1 AND t.batal=0 AND t.tbayar>'$takhir')OR(r.status_bayar=0))GROUP BY r.nomor ORDER BY m.nolama;";																			
										$qry=mysql_query($sql)or die("err: ".mysql_error());
										echo "
										<tr align=\"center\" style=\"background-color:#f0f0f0;color:#000;font-weight:bold;\">
											<td>No</td>
										...
										//```````````````````````````BOF OPTIONAL FOR FOOTER`````````````````````````````````
										$sql_jml="SELECT format(COUNT(r.periode),0),format(SUM(r.pakai),0),format(SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)),0) 
										FROM rekening1 AS r
										INNER JOIN master AS m ON m.nomor=r.nomor
										LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode
										WHERE $slipit r.nomor=m.nomor and (r.periode between $periode and $periode2) AND ((r.status_bayar=1 AND t.batal=0 AND t.tbayar>'$takhir')OR(r.status_bayar=0))";
								...
								//	$takhir='2017-04-11';
									echo"<script type=\"text/javascript\">document.getElementById(\"txtakhir\").value=\"$key2\";</script>";
										...
										$sql="SELECT DISTINCT m.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,FORMAT(COUNT(*),0) as jml_rek,FORMAT(SUM(r.pakai),0) as jpakai,FORMAT(SUM(r.uangair),0) as juangair,FORMAT(SUM(r.adm),0) as jadm,FORMAT(SUM(r.meter),0)as jmeter,FORMAT(SUM(r.meterai),0) as jmeterai,FORMAT(SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)),0) as jtotal,FORMAT(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0)as avg_harga, FORMAT(IFNULL((SUM(r.pakai)/COUNT(*)),0),0)as avg_plg 
										FROM rekening1 AS r
										INNER JOIN master AS m ON m.nomor=r.nomor
										LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode
										WHERE $slipit r.nomor=m.nomor and (r.periode between $periode and $periode2) AND ((r.status_bayar=1 AND t.batal=0 AND t.tbayar>'$takhir')OR(r.status_bayar=0))GROUP BY m.gol;";
										...
										INNER JOIN master AS m ON m.nomor=r.nomor
										LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode
										WHERE $slipit r.nomor=m.nomor and (r.periode between $periode and $periode2) AND ((r.status_bayar=1 AND t.batal=0 AND t.tbayar>'$takhir')OR(r.status_bayar=0))ORDER BY m.gol;";
