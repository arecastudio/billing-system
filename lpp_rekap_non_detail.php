        ...
        <table width="1024px" align="center" border="0" cellpadding="0" cellspacing="0" style="box-shadow: 0 0 15px #999;" background="img/grids.gif">
						...
						<table width="100%" border="1" cellpadding="1" cellspacing="1" align="center" bgcolor="white" style="border-collapse:collapse;background:#ffc;font-family:calibri;font-size:12px;">
                            ...
                            if(isset($_POST['proses'])&&$_POST['proses']=='Proses'){							
								...
								<tr align=\"center\" style=\"background-color:#f0f0f0;color:#000;font-weight:bold;\">
									<td>No.</td>
									<td>KODE<br>TARIF</td>
									<td width=180>GOLONGAN<br>TARIF</td>
									<td>JML REK AIR</td>
									<td>PAKAI AIR<br> [M<sup>3</sup>]</td>
									<td>HARGA AIR<br> [Rp.]</td>
									<td>ADMINISTRASI</td>
									<td>BIAYA METER</td>											
									<td>BIAYA METERAI</td>
								...
								$sql_i[1]="SELECT DISTINCT m.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,COUNT(*) as jml_rek,SUM(r.pakai) as jpakai,SUM(r.uangair) as juangair,SUM(r.adm) as jadm,SUM(r.meter) as jmeter,SUM(r.meterai) as jmeterai,SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)) as jtotal,ROUND(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0) as avg_harga, ROUND(IFNULL((SUM(r.pakai)/COUNT(*)),0),0) as avg_plg,r.periode,SUM(r.denda) as jdenda
								FROM rekening1 AS r
								INNER JOIN master AS m ON m.nomor=r.nomor
								INNER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode
								WHERE $selip r.periode=$bln1 AND (t.tbayar BETWEEN '$tawal' AND '$takhir') AND r.status_bayar=1 AND t.batal=0
								GROUP BY m.gol
								;";
								//$sql_i[2]="SELECT DISTINCT r.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,COUNT(*) as jml_rek,SUM(r.pakai) as jpakai,SUM(r.uangair) as juangair,SUM(r.adm) as jadm,SUM(r.meter) as jmeter,SUM(r.meterai) as jmeterai,SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)) as jtotal,ROUND(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0) as avg_harga, ROUND(IFNULL((SUM(r.pakai)/COUNT(*)),0),0) as avg_plg FROM rekening1 AS r LEFT OUTER JOIN master AS m ON m.nomor=r.nomor LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode WHERE (r.periode BETWEEN $bln3 AND $bln2) AND (r.tbayar BETWEEN '$tawal' AND '$takhir') AND r.status_bayar=1 GROUP BY r.gol;";
								//$sql_i[3]="SELECT DISTINCT r.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,COUNT(*) as jml_rek,SUM(r.pakai) as jpakai,SUM(r.uangair) as juangair,SUM(r.adm) as jadm,SUM(r.meter) as jmeter,SUM(r.meterai) as jmeterai,SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)) as jtotal,ROUND(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0) as avg_harga, ROUND(IFNULL((SUM(r.pakai)/COUNT(*)),0),0) as avg_plg FROM rekening1 AS r LEFT OUTER JOIN master AS m ON m.nomor=r.nomor LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode WHERE (r.periode BETWEEN $bln6 AND $bln4) AND (r.tbayar BETWEEN '$tawal' AND '$takhir') AND r.status_bayar=1 GROUP BY r.gol;";
								//$sql_i[4]="SELECT DISTINCT r.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,COUNT(*) as jml_rek,SUM(r.pakai) as jpakai,SUM(r.uangair) as juangair,SUM(r.adm) as jadm,SUM(r.meter) as jmeter,SUM(r.meterai) as jmeterai,SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)) as jtotal,ROUND(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0) as avg_harga, ROUND(IFNULL((SUM(r.pakai)/COUNT(*)),0),0) as avg_plg FROM rekening1 AS r LEFT OUTER JOIN master AS m ON m.nomor=r.nomor LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode WHERE (r.periode BETWEEN $bln12 AND $bln7) AND (r.tbayar BETWEEN '$tawal' AND '$takhir') AND r.status_bayar=1 GROUP BY r.gol;";
								
								//$sql_i[5]="SELECT DISTINCT r.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,COUNT(*) as jml_rek,SUM(r.pakai) as jpakai,SUM(r.uangair) as juangair,SUM(r.adm) as jadm,SUM(r.meter) as jmeter,SUM(r.meterai) as jmeterai,SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)) as jtotal,ROUND(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0) as avg_harga, ROUND(IFNULL((SUM(r.pakai)/COUNT(*)),0),0) as avg_plg FROM rekening1 AS r LEFT OUTER JOIN master AS m ON m.nomor=r.nomor LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode WHERE (r.periode BETWEEN $bln24 AND $bln13) AND (r.tbayar BETWEEN '$tawal' AND '$takhir') AND r.status_bayar=1 GROUP BY r.gol;";
								//$sql_i[6]="SELECT DISTINCT r.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,COUNT(*) as jml_rek,SUM(r.pakai) as jpakai,SUM(r.uangair) as juangair,SUM(r.adm) as jadm,SUM(r.meter) as jmeter,SUM(r.meterai) as jmeterai,SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)) as jtotal,ROUND(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0) as avg_harga, ROUND(IFNULL((SUM(r.pakai)/COUNT(*)),0),0) as avg_plg FROM rekening1 AS r LEFT OUTER JOIN master AS m ON m.nomor=r.nomor LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode WHERE (r.periode BETWEEN $bln36 AND $bln25) AND (r.tbayar BETWEEN '$tawal' AND '$takhir') AND r.status_bayar=1 GROUP BY r.gol;";
								//$sql_i[7]="SELECT DISTINCT r.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,COUNT(*) as jml_rek,SUM(r.pakai) as jpakai,SUM(r.uangair) as juangair,SUM(r.adm) as jadm,SUM(r.meter) as jmeter,SUM(r.meterai) as jmeterai,SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)) as jtotal,ROUND(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0) as avg_harga, ROUND(IFNULL((SUM(r.pakai)/COUNT(*)),0),0) as avg_plg FROM rekening1 AS r LEFT OUTER JOIN master AS m ON m.nomor=r.nomor LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode WHERE (r.periode BETWEEN $prd_batas AND $bln37) AND (r.tbayar BETWEEN '$tawal' AND '$takhir') AND r.status_bayar=1 GROUP BY r.gol;";
								
								$sql_i[2]="
								SELECT DISTINCT m.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,COUNT(*) as jml_rek,SUM(r.pakai) as jpakai,SUM(r.uangair) as juangair,SUM(r.adm) as jadm,SUM(r.meter) as jmeter,SUM(r.meterai) as jmeterai,SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)) as jtotal,ROUND(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0) as avg_harga, ROUND(IFNULL((SUM(r.pakai)/COUNT(*)),0),0) as avg_plg,r.periode,SUM(r.denda) as jdenda
								FROM rekening1 AS r
								INNER JOIN master AS m ON m.nomor=r.nomor
								INNER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode
								WHERE $selip (t.tbayar BETWEEN '$tawal' AND '$takhir') AND r.status_bayar=1 AND t.batal=0 AND (r.periode<$bln1 AND r.periode>$prd_batas)
								GROUP BY m.gol
								ORDER BY m.gol ASC;
								";
								
								//$sql_i[3]="SELECT DISTINCT m.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,COUNT(*) as jml_rek,SUM(r.pakai) as jpakai,SUM(r.uangair) as juangair,SUM(r.adm) as jadm,SUM(r.meter) as jmeter,SUM(r.meterai) as jmeterai,SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)) as jtotal,ROUND(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0) as avg_harga, ROUND(IFNULL((SUM(r.pakai)/COUNT(*)),0),0) as avg_plg,r.periode FROM rekening1 AS r INNER JOIN master AS m ON m.nomor=r.nomor LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode WHERE $selip r.periode<=$prd_batas AND (r.tbayar BETWEEN '$tawal' AND '$takhir') AND r.status_bayar=1 AND t.batal=0 GROUP BY m.gol;";
								//$sql_i[7]="SELECT DISTINCT m.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,COUNT(*) as jml_rek,SUM(r.pakai) as jpakai,SUM(r.uangair) as juangair,SUM(r.adm) as jadm,SUM(r.meter) as jmeter,SUM(r.meterai) as jmeterai,SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)) as jtotal,ROUND(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0) as avg_harga, ROUND(IFNULL((SUM(r.pakai)/COUNT(*)),0),0) as avg_plg FROM rekening1 r, master m WHERE r.periode<=$bln37 AND (r.tbayar LEFT OUTER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode BETWEEN '$tawal' AND '$takhir') AND m.nomor=r.nomor and r.status_bayar=1 GROUP BY m.gol;";
								$sql_i[3]="
								SELECT DISTINCT m.gol,(select distinct t.keterangan from tarif t where t.gol=m.gol LIMIT 0,1)as ket,COUNT(*) as jml_rek,SUM(r.pakai) as jpakai,SUM(r.uangair) as juangair,SUM(r.adm) as jadm,SUM(r.meter) as jmeter,SUM(r.meterai) as jmeterai,SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)) as jtotal,ROUND(IFNULL((SUM(r.uangair)/SUM(r.pakai)),0),0) as avg_harga, ROUND(IFNULL((SUM(r.pakai)/COUNT(*)),0),0) as avg_plg,r.periode,SUM(r.denda) as jdenda
								FROM rekening1 AS r
								INNER JOIN master AS m ON m.nomor=r.nomor
								INNER JOIN transaksi AS t ON t.nomor=r.nomor AND t.periode=r.periode
								WHERE $selip (t.tbayar BETWEEN '$tawal' AND '$takhir') AND r.status_bayar=1 AND t.batal=0 AND (r.periode<=$prd_batas)
								GROUP BY m.gol
								ORDER BY m.gol ASC;
								";
								...
								$cnt1[]="";$cnt2[]="";$baris="";
								while($j<=3){
									if ($j==1) {$bts=$bln1;$peri="Bulan Berjalan";}
									if ($j==2) {$bts=$bln2." dst. ";$peri="Bulan Tunggakan";}
									if ($j==3) {$bts="Periode <u>200001</u> s/d ".$prd_batas;$peri="Rek. Dihapuskan";}
									...
									if ($j==7) {$bts=$bln37." - ".$prd_batas;$peri="37 bulan s/d batas Rek. dihapuskan";}
									if ($j==8) {$bts="Periode <u>200001</u> s/d ".$prd_batas;$peri="Rek. Lain-lain [Dihapuskan]";}
									*/
									$qry1=mysql_query($sql_i[$j])or die(mysql_error());
									$i=0;
									
									$jrek=0;$pakai=0;$harga=0;$adm=0;$meter=0;$meterai=0;$total=0;$rata1=0;$rata2=0;$denda=0;
									
									$c1++;
									
									echo"<tr style=\"background-color:#f0f0f0;color:#000;font-weight:bold;\"><td colspan=13>Periode $peri >> $bts</td></tr>";
									$baris.="<tr style=\"background-color:#f0f0f0;color:#000;font-weight:bold;\"><td colspan=13>Periode $peri >> $bts</td></tr>";
									$rows[]="<tr style=\"background-color:#f0f0f0;color:#000;font-weight:bold;\"><td colspan=13>Periode $peri >> $bts</td></tr>";
									$counter++;
									while($row=mysql_fetch_row($qry1)){
										...
										<tr>											
											...
											<th align=\"right\">".formatMoney($denda)."</th>
							...
							$sql_jml="
							SELECT format(count(*),0),format(SUM(r.pakai),0),format(SUM(r.uangair),0),format(SUM(r.adm),0),format(SUM(r.meter),0),format(SUM(r.meterai),0),format(SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)),0),format(SUM(r.denda),0) as jdenda
							FROM rekening1 AS r
							INNER JOIN master AS m ON m.nomor=r.nomor
            ...
            <tr>
                <td align="left" style="font:Verdana, Arial, Helvetica, sans-serif;font-size:9px;">
                    <a href="reports/print_report_rekap_nondetail.php" target="_blank" style="font-size:16px;font-weight:bold;color:#FF0000;">-= Cetak Report =-</a>
                </td>
            </tr> 
        </table>
        <br/>
