	...
	<tr>
		...
		<table width="850px" border="0" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF" align="center" class="wrapper"  style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:11px;box-shadow: 0 0 15px #999;background-color:#f0f0f0;" background="img/grids.gif">
			...
			<tr>
				...
				<td><input type="radio" name="r1" value="perkolek" checked="checked">
				Kolektif</td>
				<td>
				<select name="optkolek" >
							...
							<?php
								if (isset($_POST["proses"])&& $_POST["proses"]=="Proses"){
									...
									if(isset($_POST["rjrep"])&& $_POST["rjrep"]=="rjdaftar"){
										...
										if($slipit!="")$slipit.=" AND ";
										$sql="SELECT r.nomor,m.nolama,SUBSTR(m.nama,1,25),SUBSTR(lcase(m.alamat),1,25),format(COUNT(r.periode),0),format(SUM(r.pakai),0),format(SUM((r.uangair + r.adm + r.meter + r.meterai + r.denda)),0),m.kondisi_meter,SUM(r.pakai),SUM(r.uangair),SUM(r.adm),SUM(r.meter),SUM(r.meterai),SUM(r.denda)
