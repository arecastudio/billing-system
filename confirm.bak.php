	...
	<br/>
		...
		<table background="img/grids.gif" width="800px" border="0" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF" align="center" class="wrapper" style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:11px;box-shadow: 0 0 15px #999;background-color:#f0f0f0;">
			...
			<tr style="font:calibri;font-size:18px;font-weight:bold;color:#fff;background:linear-gradient(#666,#004);">
			  <td colspan="3"><center><font face="Arial,sans-serif" size="4px" style="padding:2px 0;text-shadow:4px -4px 4px red;color:#fff;font-size:19px;"><b>CETAK Konfirmasi DSMP</b></font></center></td>
			  </tr>
			<tr>
			...
			<tr>
				...
				<td><input type="radio" name="rd2" id="rdsemua" value="all" checked="checked" />&nbsp;Semua pemakaian</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td><input type="radio" name="rd2" id="rdbedakwm" value="bedakwm" />&nbsp;Beda KWM</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>
					...
					<select name="optpetugas">
						<!--option value="">-Pilih-</option-->
						<?php
                                                $sql=mysql_query("SELECT DISTINCT petugas FROM dsmp_new WHERE petugas IS NOT NULL ORDER BY petugas;");
                                                while($row=mysql_fetch_row($sql)){
		...
		<br/>
		<!--######################################################################-->
		<table background="img/grids.gif" width="1280px" align="center" border="0" cellpadding="2" cellspacing="2" style="box-shadow: 0 0 15px #999;" class="rounded">
		<tr>
		...
		<table width="100%" border="1" cellpadding="1" cellspacing="1" align="center" bgcolor="white" style="border-collapse:collapse;background:#ffc;font-family:calibri;font-size:12px;">
			...
			if(isset($_POST["proses"])){
				...
				$sql="select m.nomor,m.nolama,SUBSTR(m.nama,1,35),SUBSTR(lcase(m.alamat),1,42),m.dkd,a.angka,b.angka,(b.angka-a.angka),m.kondisi_meter,LEFT(b.kondisi_meter,1),b.keterangan,b.tgl_baca,b.tgl,b.petugas from master as m,dsmp_new as a,dsmp_new as b $slipit order by m.nolama";
				...
				$_SESSION["jumfield"]=8;
				$_SESSION["eskiel"]="select m.nomor,m.nolama,SUBSTR(lcase(m.nama),1,30),SUBSTR(lcase(m.alamat),1,30),a.angka,b.angka,(b.angka-a.angka),m.kondisi_meter from master as m,dsmp_new as a,dsmp_new as b $slipit order by m.nolama";
				$_SESSION["alinea"]=$alg;
				$_SESSION["theader"]="
