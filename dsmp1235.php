...
if (isset($_POST['save'])){
	...
	while($j<$jum){
		...
		//$ang[$j]=strip_tags($_POST['now'.$j]);
		//$nmr[$j]=strip_tags($_POST['num'.$j]);
		//$sql="update dsmp0 set angka=$lalu[$j] where nomor='$nmr[$j]' and periode=$plalu;";
	...
	while($j<$jum){
		...
		switch($kwm0[$j]){
			case"1":
				...
				break;
			case"2":
				//$sql="UPDATE master AS m INNER JOIN dsmp0 AS a ON a.nomor=m.nomor AND a.periode=$plalu INNER JOIN dsmp0 AS b ON b.nomor=m.nomor AND b.periode=$pkini SET m.kondisi_meter=$kwm0[$j],a.angka=0,b.angka=0 WHERE m.nomor='$nmr[$j]';";
				...
				break;
			case"3":
				//$sql="UPDATE master AS m INNER JOIN dsmp0 AS a ON a.nomor=m.nomor AND a.periode=$plalu SET m.kondisi_meter=$kwm0[$j],a.angka=0 WHERE m.nomor='$nmr[$j]';";
	...
	//for($i=0;$i<=$jl;$i++){
		...
		if($kn1=='1'){
			...
			$q1="INSERT INTO dsmp_new(nomor,periode,angka)VALUES('$nr1',$plalu,0) ON DUPLICATE KEY UPDATE angka=0;";
			$q2="INSERT INTO dsmp_new(nomor,periode,angka)VALUES('$nr1',$pkini,$pk1) ON DUPLICATE KEY UPDATE angka=$
			//echo"tigaempat ".++$i;
		}
				...
				<td colspan="4" align="center" bgcolor="#0f0">
					...
					<table width="100%" border="1" bordercolor="white" style="border-collapse:collapse;background:#ffc;font-family:calibri;font-size:11px;">
						...
						if(isset($_POST["proses"])){
							...
							//$sql="SELECT DISTINCT m.nomor,m.nolama,m.nama,a.angka,b.angka,m.dkd,m.kondisi_meter FROM master AS m,dsmp0 AS a,dsmp0 AS b $slipit ;";
							//echo $sql;
							$sql="
