		...
		<table width="1045px" border="0" cellpadding="1" cellspacing="1" align="center" class="wrapper"  style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:11px;box-shadow: 0 0 15px #999;background-color:#f0f0f0;" background="img/grids.gif">
          <form id="fr1" name="fr1" method="post" action="" onSubmit="">
            <tr style="font:calibri;font-size:18px;font-weight:bold;color:#fff;background:linear-gradient(#666,#004);">
            	<td colspan="4" align="center"><marquee behavior="alternate" scrollamount="2" hspace="280">.::Pencetakan Rekening Kolektif::.</marquee></td>
            </tr>
            <tr>
              ...
              <!--select name="optprd" id="optprd" -->
                  	<?php
						/*$sql=mysql_query("SELECT DISTINCT periode FROM rekening1_faktur_desember ORDER BY periode DESC;");						
			  ...
			  &nbsp;
			  &nbsp;
			  <input type="checkbox" name="chk_periode" id="chk_periode" />Sort by Nomor
			  
			  &nbsp;
			  &nbsp;
			  <input type="checkbox" name="chk_lunas" id="chk_lunas" />Tampilkan Rek. Lunas juga
              </td>
			...
			<tr height="35px">
				...
				<select name="optperiode" id="optperiode">
                  	...
                  	<?php
						$sql=mysql_query("select distinct periode from rekening1_faktur_desember order by periode DESC;");
						//$sql=mysql_query("select distinct max(periode) from rekening1_faktur_desember;");
                        ...
                        <table width="100%" border="0" cellpadding="1" cellspacing="1" align="center" bgcolor="white" style="border-collapse:collapse;background:#ffd;font-family:calibri;font-size:12px;" >
							...
							<?php
								if (isset($_POST["proses"])&& $_POST["proses"]=="Tampilkan!" && $dapat==1){
									...
									$sort_by_periode="r.periode ASC,r.nomor ASC";
									#$chk_periode=$_POST["chk_periode"];
									if(isset($_POST['chk_periode'])){
										echo"<script type=\"text/javascript\">document.getElementById(\"chk_periode\").checked=\"checked\";</script>";
										$sort_by_periode="r.nomor ASC,r.periode ASC";
									}
									
									$show_paid="0";
									#$chk_lunas=$_POST["chk_lunas"];
									if(isset($_POST['chk_lunas'])){
										echo"<script type=\"text/javascript\">document.getElementById(\"chk_lunas\").checked=\"checked\";</script>";
										$show_paid="0,1";
									}
									...
									FROM rekening1_faktur_desember AS r
									INNER JOIN master AS m ON m.nomor=r.nomor
									INNER JOIN pel_kolektif_d AS p ON p.nomor=r.nomor
									WHERE p.kode_kolektif='$kokol' AND (r.status_bayar IN (".$show_paid.")) AND (r.periode BETWEEN '$prd1' AND '$prd2')
									ORDER BY r.nolama ASC,r.periode ASC ;"; //".$sort_by_periode." ;"; //ORDER BY r.nomor ASC,r.periode ASC//ORDER BY r.nomor ASC,r.periode ASC // ORDER BY m.cabang ASC, m.nama ASC untuk MBR
...
if (isset($_POST["lunaskan"]) && trim($_POST['txnomor'])!=""){	
	...
	//$prd=$_POST['optprd'];
	/*$sql="SELECT DISTINCT r.nomor,r.periode,(r.uangair + r.adm + r.meter + r.meterai + r.denda),m.nolama FROM rekening1_faktur_desember AS r INNER JOIN master AS m ON m.nomor=r.nomor WHERE (r.nomor='$nomor') AND r.status_bayar=0;";
	...
	while($row=mysql_fetch_row($query)){
		...
		$nmr1=trim($row[0]);		
		if(isset($_POST[$prd])){
			mysql_query("UPDATE rekening1_faktur_desember SET status_bayar=1,tbayar='".date('Y-m-d')."',loket='".$_SESSION['nama_komputer']."' WHERE nomor=$nomor AND periode=$prd;")or die(mysql_error());
			$insert="
	...
	INNER JOIN pel_kolektif_d AS p ON p.nomor=m.nomor
	INNER JOIN pel_kolektif AS k ON k.kode_kolektif=p.kode_kolektif
	WHERE p.kode_kolektif='$kokol' AND $sisip	
	ORDER BY r.periode ASC,r.nomor ASC
	;";
	...
	FROM rekening1_faktur_desember AS r	
	INNER JOIN master AS m ON m.nomor=r.nomor	
	WHERE $sisip AND r.status_bayar=0	
	ORDER BY r.periode ASC,r.nomor ASC
	;";//untuk kolektif
	...
	;";//untuk kolektif //ORDER BY r.periode ASC,r.nomor ASC
//unutk aslinya ORDER BY r.periode ASC, m.nolama ASC // untuk MBR ORDER BY m.cabang ASC, m.nama ASC
	
	$sql_rek007777777="
	SELECT DISTINCT m.nomor,m.nolama,SUBSTR(m.nama,1,30),SUBSTR(m.alamat,1,22),m.gol,(SELECT DISTINCT c.keterangan FROM tarif AS c WHERE c.gol=m.gol LIMIT 0,1) AS ket,r.standlalu,r.standkini,r.pakai,FORMAT(r.uangair,0),FORMAT(r.adm,0),FORMAT(r.meter,0),FORMAT(r.denda,0),FORMAT(r.meterai,0),(r.uangair + r.adm + r.meter + r.meterai + r.denda),r.periode,r.status_bayar,r.tbayar,r.tbayar
	FROM rekening1_faktur_desember AS r	
	INNER JOIN master AS m ON m.nomor=r.nomor	
	WHERE m.cabang='06' AND r.periode=201501 AND r.status_bayar=0	
	ORDER BY r.periode ASC,r.nomor ASC
	;";// untuk rekening genyem
	
	$sql_rek00="
	SELECT DISTINCT m.nomor,m.nolama,SUBSTR(m.nama,1,30),SUBSTR(m.alamat,1,22),m.gol,(SELECT DISTINCT c.keterangan FROM tarif AS c WHERE c.gol=m.gol LIMIT 0,1) AS ket,r.standlalu,r.standkini,r.pakai,FORMAT(r.uangair,0),FORMAT(r.adm,0),FORMAT(r.meter,0),FORMAT(r.denda,0),FORMAT(r.meterai,0),(r.uangair + r.adm + r.meter + r.meterai + r.denda),r.periode,r.status_bayar,r.tbayar,r.tbayar
	FROM rekening1_faktur_desember AS r	
	INNER JOIN master AS m ON m.nomor=r.nomor	
	WHERE m.cabang='06' AND r.periode=201501
	ORDER BY r.periode ASC,r.nomor ASC
	;";// untuk rekening genyem
