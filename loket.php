...
//if($nloket=="192.168.0.12")
$nloket="JPR";

$dapat=0;$cek="";

//echo setDenda('008068',201712);
//echo getDenda('008068');
...
if (isset($_POST["proses"])&& $_POST["proses"]=="Tampilkan!"){
	...
	WHERE (nomor='$nomor' OR nolama='$nomor') AND putus=0
	;";	
		...
		<table width="1045px" border="0" cellpadding="1" cellspacing="1" align="center" class="wrapper"  style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:11px;box-shadow: 0 0 15px #999;background-color:#f0f0f0;" background="img/grids.gif">
          <form id="fr1" name="fr1" method="post" action="" onSubmit="">
            ...
            <tr>
              ...
              <td width="500px"><input type="text" size="13" name="txnomor" id="txnomor" maxlength="15"  onkeypress="return isNumb(event);" />
              <input type="button" name="cari" value="Cari!" class="kelas_tombol" onclick="window.open('cari.php','popUpWindow','height=400,width=800,left=10,top=10,scrollbars=yes,menubar=no'); return false;" /></td>
			...
			<tr height="35px">
                ...
                <td>
                    ...
                    <input type="radio" name="rcek" id="none" value="" checked /> Un-Check All               	</td>
								...
								if (isset($_POST["proses"])&& $_POST["proses"]=="Tampilkan!" && $dapat==1){
									...
									while($rows=mysql_fetch_row($qry)){
										...
										<tr bgcolor=\"$wrn\">
											...
											<td align=\"right\">".formatMoney($rows[7]+$dnd)."</td>
											<td align=\"right\">".formatMoney($rows[8])."</td>
											<td align=\"right\" style=\"color:#00f;font-weight:bold;\">".formatMoney($rows[9]+$dnd)."</td>
										</tr>
										";
										$total+=$rows[9]+$dnd;
									}																	
