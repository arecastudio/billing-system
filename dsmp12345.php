...
if (isset($_POST['save'])){
	...
	while($j<$jum){
		...
		VALUES('$nmr[$j]','$tgl','$jam',(SELECT DISTINCT n.kondisi_meter FROM master AS n WHERE n.nomor='$nmr[$j]' LIMIT 1),'$kwm0[$j]','$operator','$nloket')
		ON DUPLICATE KEY UPDATE baru='$kwm0[$j]',jam='$jam',operator='$operator',loket='$nloket'
		;";
		
		$q2="
		INSERT INTO ubah_kwm(nomor,tgl,jam,lama,baru,operator,loket)
		SELECT * FROM (SELECT '$nmr[$j]','$tgl','$jam','$kwm_lama[$j]','$kwm0[$j]','$operator','$nloket') AS nilai		
		ON DUPLICATE KEY UPDATE baru='$kwm0[$j]',jam='$jam',operator='$operator',loket='$nloket'
		;";
