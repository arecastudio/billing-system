...
?>
        ...
        if (isset($_POST['simpan'])&&$_POST['simpan']=='Simpan'){
			...
			for($j=0;$j<=$jdt;$j++){
				...
				$hp='hdp'.$j;
				if(isset($_POST[$ck])){
					$sql1="UPDATE rekening1 SET status_bayar=1 WHERE nomor='".$_POST[$hn]."' AND periode=".$_POST[$hp].";";
	...
	<tr>
		...
		<center><font face="Arial,sans-serif" size="4px"><b>Pembayaran Rekening Kolektif [Faktur]</b></font></center><hr width="900px"/>
		...
		<table width="900px" align="center" border="0" cellpadding="0" cellspacing="0" style="box-shadow: 0 0 15px #999;" background="img/grids.gif">
			<form id="fentry" name="fentry" method="post" action="">
            <tr>
								...
								if($kode!=""&&$tbyr!=""){
									...
									while($row=mysql_fetch_row($qry)){
										...
										<tr>
											...
											<td align=\"center\">$row[0]<input type=\"hidden\" name=hdn".$i." value=$row[0] /></td>
											<td align=\"center\">$row[1]</td>
											<td>$row[2]</td>
											<td align=\"center\"><input type=\"checkbox\" name=ck".$i." value=\"\" checked /></td>
											<td align=\"center\">$row[3]<input type=\"hidden\" name=hdp".$i." value=$row[3] /></td>
									...
									$_SESSION['jdata']=$i;
									echo"<input type=\"hidden\" name=\"jmldt\" value=$i />";
									//echo $sql1;
								}
							}
                            ?>
