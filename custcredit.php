            ...
            <table width="880px" border="0" cellpadding="1" cellspacing="2" align="center" class="wrapper"  style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:12px;box-shadow: 0 0 15px #999;" background="img/grids.gif">
                ...
                if(isset($_POST['cari']) && $_POST['txnomor']!=''){
					...
					if($row=mysql_fetch_row($qry)){
						...
						$sql="SELECT DISTINCT r.periode, FORMAT(r.standlalu,0), FORMAT(r.standkini,0), FORMAT (r.pakai,0), FORMAT(r.uangair,0), FORMAT(r.adm,0), FORMAT(r.meter,0), FORMAT(r.denda,0), IFNULL(FORMAT(r.meterai,0),0), IFNULL(FORMAT(r.uangair + r.adm + r.meter + r.meterai + r.denda,0),0),IF(r.status_bayar=0,'TUNGGAK','LUNAS'),IF(r.status_bayar=1,(SELECT t.tbayar FROM transaksi AS t WHERE t.nomor=r.nomor AND t.periode=r.periode ORDER BY t.batal ASC,t.tbayar ASC, t.jbayar ASC LIMIT 1),'-'),IF(r.status_bayar=1,(SELECT t.loket FROM transaksi AS t WHERE t.nomor=r.nomor AND t.periode=r.periode ORDER BY t.batal ASC,t.tbayar ASC, t.jbayar ASC LIMIT 1),'-'),IF(r.status_bayar=1,(SELECT t.operator FROM transaksi AS t WHERE t.nomor=r.nomor AND t.periode=r.periode ORDER BY t.batal ASC,t.tbayar ASC, t.jbayar ASC LIMIT 1),'-'),r.status_bayar
						FROM rekening1 AS r
