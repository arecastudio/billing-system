...
if (isset($_POST["proses"])&& $_POST["proses"]=="Tampilkan!"){
	...
	$sql="
	SELECT kode_kolektif, ket_kolektif, alamat_kolektif, kode_cabang,(SELECT DISTINCT c.nama FROM cabang AS c WHERE c.cabang=kode_cabang LIMIT 1), kode_wilayah, kode_blok, dkd, penanggung_jawab, telp
		...
		<table width="1045px" border="0" cellpadding="1" cellspacing="1" align="center" class="wrapper"  style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:11px;box-shadow: 0 0 15px #999;background-color:#f0f0f0;" background="img/grids.gif">
          <form id="fr1" name="fr1" method="post" action="" onSubmit="">
            <tr style="font:calibri;font-size:18px;font-weight:bold;color:#fff;background:linear-gradient(#666,#004);">
            	<td colspan="4" align="center"><marquee behavior="alternate" scrollamount="2" hspace="280">.::Pelunasan Rekening Kolektif::.</marquee></td>
            </tr>
            <tr>
              <td width="130px">Kode Kolektif</td>
              <td width="500px"><input type="text" size="4" name="txnomor" id="txnomor" maxlength="3"  onkeypress="return isNumb(event);" />
              <!-- &nbsp;&nbsp;Periode&nbsp;:&nbsp; -->
              <!--select name="optprd" id="optprd" -->
                  	<?php
					...
					?>
                <!--/select-->
              <!--nput type="button" name="cari" value="Cari!" class="kelas_tombol" onclick="window.open('cari.php','popUpWindow','height=400,width=800,left=10,top=10,scrollbars=yes,menubar=no'); return false;" /-->
			  &nbsp;&nbsp;&nbsp;&nbsp;
			  Loket&nbsp;:&nbsp;
				<select name="optjlok" id="optjlok">
					<!--option value="">-Pilih-</option-->
					<?php
						$sql=mysql_query("select kode_loket,ket_loket from m_loket where kode_loket not in('ABE','WNA','JPR','UJU','STN','BPD','POS','BRV','BNI') order by kode_loket");
						while($row=mysql_fetch_row($sql)){
                ...
                &nbsp;&nbsp;
                <select name="optkolek" id="optkolek" onchange="return changeValue(this.value)">
				  ...
				  function changeValue(kode_kolektif){
				  document.getElementById('txnomor').value=kode_kolektif;
				  //document.getElementByld('txnomor').value=;
				  //alert(kode_kolektif);
				  }
				  </script>
              ...
              <td style="font-size:18px;font-family:calibri;font-weight:bold;"><?php if(isset($_POST["proses"]) && $dapat==1){echo $res;}?></td>              
            </tr>
            <tr>
            	<td style="font-size:18px;font-family:calibri;font-weight:bold;">Keterangan</td>
                <td style="font-size:18px;font-family:calibri;font-weight:bold;"><?php if(isset($_POST["proses"])  && $dapat==1){echo $kokol."/".$ket;}?></td>
                <td>No. Telp</td>
              	<td><?php if(isset($_POST["proses"]) && $dapat==1){echo $tlp;}?></td>
            </tr>
            <tr>
            	<td>Cabang</td>
            ...
            <tr>
            	<td>Wilayah</td>
                <td><?php if(isset($_POST["proses"]) && $dapat==1){echo $wil;}?></td>
                <td>Blok</td>
              	<td><?php if(isset($_POST["proses"]) && $dapat==1){echo $blok;}?></td>
            </tr>
			<tr height="35px">
				<td>
					<input type="submit" name="proses" value="Tampilkan!" class="kelas_tombol" />                </td>
                <td>
                ...
                <td>
					<input type="text" size="11" maxlength="10" name="txtawal" id="txtawal" style="font-size:18px;font-family:calibri;font-weight:bold;" class="datepicker" value="<?php #echo date('Y-m-d');?>"  />				</td>
			</tr>
          <!--/form-->
		  </table>
		  ...
		  <table width="1054px" align="center" border="0" cellpadding="2" cellspacing="1" class="rounded" background="img/grids.gif">			
                        ...
                        <table width="100%" border="0" cellpadding="1" cellspacing="1" align="center" bgcolor="white" style="border-collapse:collapse;background:#ffd;font-family:calibri;font-size:12px;" >
							...
							<?php
								if (isset($_POST["proses"])&& $_POST["proses"]=="Tampilkan!" && $dapat==1){
									...
									#echo"<script type=\"text/javascript\">document.getElementById(\"txtawal\").value=\"$tgl\";</script>";
									
									$lkt=$_POST['optjlok'];
									echo"<script type=\"text/javascript\">document.getElementById(\"optjlok\").value=\"$lkt\";</script>";
									...
									WHERE p.kode_kolektif='$kokol' AND r.status_bayar=0 AND (r.periode BETWEEN '$prd1' AND '$prd2')
									ORDER BY m.nolama ASC,r.periode ASC,r.nomor ASC
									;";
									...
									while($rows=mysql_fetch_row($qry)){
										...
										<tr bgcolor=\"$wrn\">
											...
											<td align=\"center\" style=\"color:#f00;font-weight:bold;font-size:16px;\">".$rows[0]."</td>
										...
										echo"<input type=\"hidden\" name=\"txhtotal$i\" id=\"txhtotal$i\" value=\"$rows[9]\" />";
			...
			<tr height="25px">
                <td align="left" style="font:Verdana, Arial, Helvetica, sans-serif;font-size:12px;" rowspan="2">
                    ...
                    ?>
                    <input type="submit" name="lunaskan" value="Lunaskan!" class="kelas_tombol"/>
                </td>
...
if (isset($_POST["lunaskan"]) && trim($_POST['txnomor'])!=""){	
	...
	$loket=trim($_POST['optjlok']);
	$tbayar=trim($_POST['txtawal']);
	
echo"<script type=\"text/javascript\">document.getElementById('txtawal').value=\"$tbayar\";</script>";
echo"<script type=\"text/javascript\">document.getElementById('txnomor').value=\"$kokol\";</script>";
	...
	for($i=1;$i<=$jdata;$i++){
		if(isset($_POST["chk_".$i])){
			...
			if($injek!="")$injek.=", ";
			$injek.="('".$_POST["nmr".$i]."',".$_POST["prd".$i].",".$_POST["txhtotal".$i].",'$tbayar','".date('H:i:s')."','$loket','$operator',0)";
		}
	}
	
	//ON DUPLICATE KEY UPDATE nomor=values(nomor),periode=values(periode),total=values(total),tbayar=values(tbayar),jbayar=values(jbayar),loket=values(loket),operator=values(operator),batal=values(batal)
	...
	SET r.status_bayar=1,r.tbayar='$tbayar',r.loket='$loket'
	WHERE p.kode_kolektif='$kokol' AND $sisip AND r.status_bayar=0
	;";
	
	
	
	mysql_query($sql_rek)or die("err_rek: ".mysql_error());	
	mysql_query($sql_tra)or die("err_tra: ".mysql_error());
