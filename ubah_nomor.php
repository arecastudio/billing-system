...
kunci_halaman("ubah_no_pel",$secret,$operator);
?>
		...
		<table background="img/grids.gif" width="500px" border="0" cellpadding="2" cellspacing="1" bgcolor="#FFFFFF" align="center" class="wrapper" style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:11px;box-shadow: 0 0 15px #999;background-color:#f0f0f0;">
			<form id="fr1" name="fr1" method="post" action="" onSubmit="return suvalid(this);">
				<tr style="font:calibri;font-size:18px;font-weight:bold;color:#fff;background:linear-gradient(#666,#004);">
				  <td colspan="3"><center><font face="Arial,sans-serif" size="4px" style="padding:2px 0;text-shadow:4px -4px 4px red;color:#fff;font-size:19px;"><b>Perubahan Nomor ID Pelanggan</b></font></center></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td colspan="2">
						...
						if(isset($_POST['cari']) && $_POST['txnomor']!=''){
							...
							echo"<script type=\"text/javascript\">document.getElementById(\"txhlama\").value=\"$row[1]\";</script>";
...
<td><?php sumber_ubah();?></td>
		...
		<?php			
			...
			if(isset($_POST['save'])){
				...
				$q1="
				UPDATE master
				SET nolama='$snama', nu='$tgl'
				WHERE nomor='$snomor'
				;";
				mysql_query($q1)or die(mysql_error());
				
				$q2="
				INSERT INTO ubah_nomor(nomor,tgl,jam,lama,baru,operator,loket,sumber)
