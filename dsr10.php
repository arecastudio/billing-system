	...
	<tr>
		...
		<table width="850px" border="0" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF" align="center" class="wrapper"  style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:11px;box-shadow: 0 0 15px #999;background-color:#f0f0f0;" background="img/grids.gif">
			...
			<tr>
				...
				<td width="370"><select name="optperiode1" >
				  ...
				  <?php
						...
						//if($row=mysql_fetch_row($sql)){
							...
							echo"<option value=\"200001\" selected=\"selected\">200001</option>";
...
Kondisi WM</td>
			...
			<tr>
				...
				<td align="right">
					...
					<input type="button" name="batal" value="Batal" onclick="location.header('drd.php');" />				</td>
			</tr>
			</form>
		</table>
		<br />
							...
							<?php
								if (isset($_POST["proses"])&& $_POST["proses"]=="Proses"){
									...
									if(isset($_POST["rjrep"])&& $_POST["rjrep"]=="rjperpel"){
										...
										$nopel="";
										$sql0="
										SELECT m.nomor,m.nolama,m.nama,m.alamat,m.kondisi_meter,r.periode,r.pakai,r.uangair,r.adm,r.meter,r.denda,r.meterai,r.total
										FROM master AS m
										INNER JOIN rekening1 AS r ON r.nomor=m.nomor
										WHERE $slipit (r.periode BETWEEN $periode AND $periode2) AND m.putus=0 AND r.status_bayar=0 AND m.putus=0
									...
									if(isset($_POST["rjrep"])&& $_POST["rjrep"]=="rjperpel666"){
										...
										if($slipit!="")$slipit.=" AND ";
										//$sql="SELECT r.nomor,m.nolama,m.nama,r.periode,format(r.uangair,0),format(r.adm,0),format(r.meter,0),format(r.meterai,0),format(r.denda,0),format(r.total,0) FROM rekening1 r, master m WHERE $slipit r.nomor=m.nomor and (r.periode BETWEEN $periode AND $periode2) and r.status_bayar=0 ORDER BY m.nolama;";
										$sql="SELECT r.nomor,m.nolama,SUBSTR(m.nama,1,25),r.periode,format(r.uangair,0),format(r.adm,0),format(r.meter,0),format(r.meterai,0),format(r.denda,0),format(r.total,0)
										FROM rekening1 AS r
										INNER JOIN master AS m ON m.nomor=r.nomor
										WHERE $slipit (r.periode BETWEEN $periode AND $periode2) and r.status_bayar=0 AND m.putus=0
										ORDER BY m.nolama;";																				
										...
										$sql_jml="SELECT format(SUM(r.uangair),0),format(SUM(r.adm),0),format(SUM(r.meter),0),format(SUM(r.meterai),0),format(SUM(r.denda),0),format(SUM(r.total),0) FROM rekening1 r, master m WHERE $slipit r.nomor=m.nomor and (r.periode between $periode and $periode2) and r.status_bayar=0 ORDER BY m.nolama;";
									...
									if(isset($_POST["rjrep"])&& $_POST["rjrep"]=="rjdaftar"){
										...
										if($slipit!="")$slipit.=" AND ";
										$sql="SELECT r.nomor,m.nolama,SUBSTR(m.nama,1,25),SUBSTR(lcase(m.alamat),1,25),format(COUNT(r.periode),0),format(SUM(r.pakai),0),format(SUM(r.total),0),m.kondisi_meter
										FROM rekening1 r, master m
										WHERE $slipit r.nomor=m.nomor and (r.periode BETWEEN $periode AND $periode2) and r.status_bayar=0 AND m.putus=0
										...
										//```````````````````````````BOF OPTIONAL FOR FOOTER`````````````````````````````````
										$sql_jml="SELECT format(COUNT(r.periode),0),format(SUM(r.pakai),0),format(SUM(r.total),0) FROM rekening1 r, master m WHERE $slipit r.nomor=m.nomor and (r.periode between $periode and $periode2) and r.status_bayar=0 AND m.putus=0;";
									...
									if(isset($_POST["rjrep"])&& $_POST["rjrep"]=="rjrekap"){
										...
										//```````````````````````````BOF OPTIONAL FOR FOOTER`````````````````````````````````
										$sql_jml="SELECT format(count(*),0),format(SUM(r.pakai),0),format(SUM(r.uangair),0),format(SUM(r.adm),0),format(SUM(r.meter),0),format(SUM(r.meterai),0),format(SUM(r.total),0) FROM rekening1 r,master m WHERE $slipit r.nomor=m.nomor and (r.periode between $periode and $periode2) and r.status_bayar=0 AND m.putus=0 ORDER BY m.gol;";
