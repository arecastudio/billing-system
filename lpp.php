          ...
          <form id="fr1" name="fr1" method="post" action="" onSubmit="">
            ...
            <tr>
              ...
              <td align="right"><div align="left">
                <input type="checkbox" name="chopr" id="chopr" />
              Per Operator </div></td>
              <td>
			  	<select name="optopr" id="optopr">
                  <!--option value="">-Pilih-</option-->
                  <?php
            ...
            <tr>
              ...
              <td><select id="optdkd" name="optdkd" onchange="setalamat();" >
                ...
                <td>
					<input type="checkbox" name="chjpel" id="chjpel" />Jns Pelanggan				</td>
				<td>
...
<input type="radio" name="rjtrans" id="rjtrans0" value="0" checked="checked" />
Lunas &nbsp;&nbsp;
							...
							<?php
								if (isset($_POST["proses"])&& $_POST["proses"]=="Proses"){
									...
									if(isset($_POST["chopr"])&&($key=$_POST['optopr'])!=''){
										...
										echo"<script type=\"text/javascript\">document.getElementById(\"chopr\").checked=\"checked\";</script>";
										echo"<script type=\"text/javascript\">document.getElementById(\"optopr\").value=\"$key\";</script>";
									...
									if(isset($_POST["rjrep"])&& $_POST["rjrep"]=="rjdetail"){
										...
										<tr align=\"center\" style=\"color:#000;font-weight:bold;\">
											...
											<!--td style=\"border:1px solid;width:70; \">Operator</td-->													</tr>
										";
										...
										$sql="
										SELECT DISTINCT z.tbayar,z.jbayar,r.nomor,m.nolama,SUBSTR(m.nama,1,25),SUBSTR(m.alamat,1,25),r.periode,(r.pakai),FORMAT(r.uangair,0),FORMAT(r.adm,0),FORMAT(r.meter,0),format(r.denda,0),format(r.meterai,0),format(r.uangair + r.adm + r.meter + r.meterai + r.denda,0),(SELECT DISTINCT t.operator FROM transaksi AS t WHERE t.periode=r.periode AND t.nomor=r.nomor AND t.batal=z.batal LIMIT 1) AS op
										FROM rekening1 AS r
									...
									//=====================================BOF=REKAP==============================================
										...
										if($periode<=201212){
											...
											LEFT OUTER JOIN master AS m ON m.nomor=r.nomor
											LEFT OUTER JOIN transaksi AS z ON z.nomor=r.nomor
											WHERE $slipit (z.tbayar BETWEEN '$tawal' AND '$takhir') AND z.periode=r.periode AND (r.periode BETWEEN $prawal AND 201212)
											GROUP BY m.gol
											ORDER BY m.gol
											;";
										}
										...
										if($ada_hapus==1){											
											...
											echo"
												<tr style=\"background-color:#ffc;color:#000;font-weight:bold;\">
													<td colspan=\"12\">Periode Rek. di bawah Tahun 2013 [200001 - 201212]</td>
												</tr>
												";
